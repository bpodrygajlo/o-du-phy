...
void xran_init_mbuf_pool(uint32_t mtu)
{
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    _eth_mbuf_pool = rte_pktmbuf_pool_create("mempool", NUM_MBUFS, MBUF_CACHE, 0, data_room_size, rte_socket_id());
    _eth_mbuf_pool_indirect = rte_pktmbuf_pool_create("mempool_indirect", NUM_MBUFS_VF, MBUF_CACHE, 0, 0, rte_socket_id());
    _eth_mbuf_pkt_gen =
        rte_pktmbuf_pool_create("mempool_pkt_gen", NUM_MBUFS, MBUF_CACHE, 0, MBUF_POOL_PKT_GEN_ELM, rte_socket_id());
  } else {
...
void xran_init_port(int p_id, uint16_t num_rxq, uint32_t mtu)
{
  ...
  struct rte_eth_rxmode rxmode = {.split_hdr_size = 0, .max_rx_pkt_len = MAX_RX_LEN, .offloads = DEV_RX_OFFLOAD_JUMBO_FRAME};
  struct rte_eth_txmode txmode = {.mq_mode = ETH_MQ_TX_NONE, .offloads = DEV_TX_OFFLOAD_MULTI_SEGS};
  ...
  for (qi = 0; qi < num_rxq; qi++) {
    ...
    _eth_mbuf_pool_vf_rx[p_id][qi] =
        rte_pktmbuf_pool_create(rx_pool_name, num_mbufs, MBUF_CACHE, 0, data_room_size, rte_socket_id());
  ...
  _eth_mbuf_pool_vf_small[p_id] =
      rte_pktmbuf_pool_create(rx_pool_name, NUM_MBUFS_VF, MBUF_CACHE, 0, MBUF_POOL_ELM_SMALL_INDIRECT, rte_socket_id());
  ...
  for (qi = 0; qi < num_rxq; qi++) {
    ret = rx_queue_setup(p_id, qi, nb_rxd, sock_id, &rxq_conf, _eth_mbuf_pool_vf_rx[p_id][qi]);
  }
...
void xran_init_port_mempool(int p_id, uint32_t mtu)
{
  ...
  printf("[%d] %s\n", p_id, rx_pool_name);
  _eth_mbuf_pool_vf_small[p_id] =
      rte_pktmbuf_pool_create(rx_pool_name, NUM_MBUFS_VF, MBUF_CACHE, 0, MBUF_POOL_ELM_SMALL, rte_socket_id());
