                      ...
                      uint16_t vf_id)
{
  ...
  for (i = 0; i < RTE_DIM(xran_ethertype_handlers); ++i) {
    if (xran_ethertype_handlers[i].ethertype == ethertype && xran_ethertype_handlers[i].fn) {
...
int32_t xran_ethdi_init_dpdk(char *name, char *vfio_name, struct xran_io_cfg *io_cfg)
{
  ...
  char *argv[14 + XRAN_MAX_AUX_BBDEV_NUM] = {name,
                                             core_mask,
                                             main_core,
                                             "-n2",
                                             iova_mode,
  ...
  printf(
      "total cores %d c_mask 0x%lx%016lx core %d [id] system_core %d [id] pkt_proc_core 0x%lx%016lx [mask] pkt_aux_core %d [id] "
      "timing_core %u [id]\n",
      coreNum,
      c_mask_64_127,
      c_mask,
  ...
  *lcore_id = rte_get_next_lcore(rte_lcore_id(), 0, 1);
  ...
  for (i = 0; i < XRAN_VF_MAX && i < io_cfg->num_vfs; i++) {
    if (io_cfg->dpdk_dev[i]) {
      ...
      if (rte_dev_probe(io_cfg->dpdk_dev[i]) != 0 || rte_eth_dev_count_avail() == 0) {
      ...
      if (!(i & 1) || io_cfg->one_vf_cu_plane) {
        ...
        ctx->tx_ring[i] = rte_ring_create(ring_name, NUM_MBUFS_RING_TRX, rte_lcore_to_socket_id(*lcore_id), RING_F_SC_DEQ);
        ...
        for (qi = 0; qi < io_cfg->num_rxq; qi++) {
          ...
          ctx->rx_ring[i][qi] = rte_ring_create(ring_name, NUM_MBUFS_RING_TRX, rte_lcore_to_socket_id(*lcore_id), RING_F_SP_ENQ);
  ...
  for (i = 0; i < XRAN_VF_MAX && i < io_cfg->num_vfs; i++) {
    if (io_cfg->dpdk_dev[i]) {
      ...
      printf("[%2d] vf %2u local  SRC MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
             (unsigned)xran_port,
             (unsigned)i,
             p_addr->addr_bytes[0],
             p_addr->addr_bytes[1],
      ...
      p_addr = &p_ru_addr[i];
      printf("[%2d] vf %2u remote DST MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
             (unsigned)xran_port,
             (unsigned)i,
             p_addr->addr_bytes[0],
             p_addr->addr_bytes[1],
...
void xran_update_eth_addr(struct xran_io_cfg *io_cfg, uint32_t RU_port, uint32_t num_vf_port, struct rte_ether_addr *p_o_ru_addr)
{
  ...
  for (i = 0; i < XRAN_VF_MAX && i < io_cfg->num_vfs; i++) {
    if (ctx->vf2xran_port[i] == RU_port) {
      ...
      printf("updating [%2d] vf %2u remote DST MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8
             "\n",
             RU_port,
             j,
             p_addr->addr_bytes[0],
 ...
 * =============================================================
 */
int32_t xran_oam_phy2xran_manage_lbm(uint8_t lbmEnable,
                                     __attribute__((unused)) uint8_t vfId,
                                     __attribute__((unused)) uint8_t param3)
{
  ...
  switch (lbm_port_info->lbm_state) {
    case XRAN_LBM_STATE_IDLE:
      ...
      if (lbm_port_info->xmitEnable == 1) {
        ...
        print_dbg("LBM transmitted waiting for LBR: port %hhu: transId ",
                  eth_port,
                  eth_ctx->lbm_port_info[eth_port].nextLBMtransID - 1);
      }
    ...
    case XRAN_LBM_STATE_WAITING:
      ...
      if ((lbm_port_info->LBRreceived) && (TIMESPEC_TO_NSEC(currTime) <= lbm_port_info->nextPass)
...
xran_status_t xran_process_lbm(struct xran_ethdi_ctx *ctx, uint8_t eth_port_start, uint8_t eth_port_end)
{
  ...
  if (unlikely(first_call == 1)) {
    ...
    if (currTime >= (ctx->lbm_common_info.nextLBMtime)) {
      ...
      ctx->lbm_common_info.nextLBMtime = currTime + (ctx->lbm_common_info.LBMPeriodicity * NSEC_PER_MILLI_SEC);
      // printf("\n1. currTime %lu LBMPeriodicity*NSEC_PER_MILLI_SEC %lu nextLBMtime %lu",currTime,
      // ctx->lbm_common_info.LBMPeriodicity*NSEC_PER_MILLI_SEC, ctx->lbm_common_info.nextLBMtime);
    }
...
{
  struct xran_ethdi_ctx *eth_ctx = xran_ethdi_get_ctx();
  uint16_t expected_tlv_length =
      sizeof(lbm_hdr->tlv.chassis_id_length) + sizeof(lbm_hdr->tlv.chassis_id_subtype) + sizeof(struct rte_ether_addr);
  ...
  if ((lbm_hdr->tlv.type != SENDER_ID_TLV) || (lbm_hdr->tlv.chassis_id_subtype != CHASSIS_ID_SUBTYPE_MAC_ADDR)
  ...
  if (rte_be_to_cpu_32(lbm_hdr->loopBackTransactionIdentifier) != eth_ctx->lbm_port_info[vf_id].expectedLBRtransID) {
    ...
    eth_ctx->lbm_port_info[vf_id].expectedLBRtransID =
        rte_be_to_cpu_32(lbm_hdr->loopBackTransactionIdentifier) + 1; // Next pkt will have + 1 transId
    print_err("OOO LBR - expected %u receieved %u",
              eth_ctx->lbm_port_info[vf_id].expectedLBRtransID,
              rte_be_to_cpu_32(lbm_hdr->loopBackTransactionIdentifier));
  } else {
...
// 2. Prepare eth header -> will be done in xran_ethdi_mbuf_send
  ...
  lbm_hdr->tlv.length = rte_cpu_to_be_16(sizeof(lbm_hdr->tlv.chassis_id_length) + sizeof(lbm_hdr->tlv.chassis_id_subtype)
                                         + sizeof(struct rte_ether_addr)); /*Size of value field */
...
static inline uint16_t xran_tx_from_ring(int port, struct rte_ring *r)
{
  ...
  dequeued = rte_ring_dequeue_burst(r, (void **)mbufs, BURST_SIZE, &remaining);
  if (!dequeued)
...
int32_t process_dpdk_io_port_id(int32_t port_start, int32_t port_num)
{
  ...
  for (port_id = port_start; port_id < XRAN_VF_MAX && port_id < ctx->io_cfg.num_vfs && port_id < (port_start + port_num);
       port_id++) {
    struct rte_mbuf *mbufs[BURST_RX_IO_SIZE];
    if (port[port_id] == 0xFF)
      return 0;
