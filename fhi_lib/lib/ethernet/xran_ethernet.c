...
extern inline int xran_enqueue_mbuf(struct rte_mbuf *mb, struct rte_ring *r);
...
void xran_init_mbuf_pool(struct xran_io_cfg *io_cfg, uint32_t mtu)
{
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    _eth_mbuf_pool = xran_pktmbuf_pool_create("mempool", io_cfg->num_mbuf_alloc, MBUF_CACHE, 0, data_room_size, rte_socket_id());
    _eth_mbuf_pool_indirect =
        xran_pktmbuf_pool_create("mempool_indirect", io_cfg->num_mbuf_vf_alloc, MBUF_CACHE, 0, 0, rte_socket_id());
    _eth_mbuf_pkt_gen =
        xran_pktmbuf_pool_create("mempool_pkt_gen", io_cfg->num_mbuf_alloc, MBUF_CACHE, 0, MBUF_POOL_PKT_GEN_ELM, rte_socket_id());
  } else {
...
int rx_queue_setup(uint16_t port_id,
...
void xran_init_port(int p_id, struct xran_io_cfg *io_cfg, uint32_t mtu)
{
  static uint16_t nb_rxd = BURST_SIZE;
  static uint16_t nb_txd = BURST_SIZE;
  uint16_t num_rxq = io_cfg->num_rxq;
  ...
  struct rte_eth_txmode txmode = {.mq_mode = RTE_ETH_MQ_TX_NONE, .offloads = RTE_ETH_TX_OFFLOAD_MULTI_SEGS};
  struct rte_eth_conf port_conf = {.rxmode = rxmode, .txmode = txmode};
  ...
  uint16_t data_room_size = MBUF_POOL_ELEMENT;
  uint16_t qi = 0;
  uint32_t num_mbufs = io_cfg->num_mbuf_alloc;

  if (mtu <= 1500) {
#if (RTE_VER_YEAR >= 21)
    rxmode.mtu = MAX_RX_LEN,
#else
  ...
  for (qi = 0; qi < num_rxq; qi++) {
    if (qi == 0) {
      ...
      _eth_mbuf_pool_vf_rx[p_id][qi] =
          xran_pktmbuf_pool_create(rx_pool_name, num_mbufs, MBUF_CACHE, 0, data_room_size, rte_socket_id());
    } else {
      _eth_mbuf_pool_vf_rx[p_id][qi] = _eth_mbuf_pool_vf_rx[p_id][0];
    }
  ...
  printf("[%d] %s\n", p_id, rx_pool_name);
  _eth_mbuf_pool_vf_small[p_id] = xran_pktmbuf_pool_create(rx_pool_name,
                                                           io_cfg->num_mbuf_vf_alloc,
                                                           MBUF_CACHE,
                                                           0,
                                                           MBUF_POOL_ELM_SMALL_INDIRECT,
                                                           rte_socket_id());
  ...
  for (qi = 0; qi < num_rxq; qi++) {
    ret = rx_queue_setup(p_id, qi, (qi == 0) ? BURST_SIZE : nb_rxd, sock_id, &rxq_conf, _eth_mbuf_pool_vf_rx[p_id][qi]);
  }
...
void xran_init_port_mempool(int p_id, struct xran_io_cfg *io_cfg)
{
  ...
  snprintf(rx_pool_name, RTE_DIM(rx_pool_name), "%s_%d", "mempool_small_", p_id);
  printf("[%d] %s\n", p_id, rx_pool_name);
  _eth_mbuf_pool_vf_small[p_id] =
      xran_pktmbuf_pool_create(rx_pool_name, io_cfg->num_mbuf_vf_alloc, MBUF_CACHE, 0, MBUF_POOL_ELM_SMALL, rte_socket_id());
