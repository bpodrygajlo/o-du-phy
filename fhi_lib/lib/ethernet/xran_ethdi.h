...
enum xran_entities_id { ID_O_DU, ID_O_RU, ID_MAX };

static char *const entity_names[] = {(char *)"ORAN O-DU sim app", (char *)"ORAN O-RU sim app"};
...
typedef int (
    *ethertype_handler)(struct rte_mbuf *pkt_q[], uint16_t xport_id, struct xran_eaxc_info *p_cid, uint16_t num, uint16_t vf_id);
