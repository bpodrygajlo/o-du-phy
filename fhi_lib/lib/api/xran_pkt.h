...
******************************************************************************
*/
typedef enum xran_cfm_opcode_e { CFM_OPCODE_LOOPBACK_REPLY = 2, CFM_OPCODE_LOOPBACK_MESSAGE = 3 } xran_cfm_opcode;
...
typedef struct __rte_packed xran_cfm_common_header_s {
  ...
  uint8_t first_tlv_offset; /* 21.4.5 - The offset, starting from the first octet following the First TLV Offset field, up to the
                               first TLV in the CFM PDU*/
...
typedef struct __rte_packed xran_sender_id_tlv_s {
  ...
  uint8_t
      chassis_id_subtype; /* 21.5.3.2 Identifies the format of the Chassis ID field. Specified by 9.5.2.2 of IEEE Std 802.1AB-2005*/
