...
#define XRAN_DEFAULT_MU \
  XRAN_MAX_NUM_MU /**< primary numerology for given ru/xranport will be used when this value is provided by L1 */
...
#define XRAN_MAX_ANTENNA_NR                          \
  (16) /**< Max number of extended Antenna-Carriers: \
...
#define XRAN_NUM_OF_ANT_RADIO \
  (XRAN_MAX_SECTOR_NR * XRAN_MAX_ANTENNA_NR) /**< Max Number of Antennas supported for all CC on single XRAN port */
...
#define XRAN_MAX_SECTIONS_PER_SLOT \
  (32) /**< Max number of different sections in single slot (section may be equal to RB allocation for UE) */
#define XRAN_MIN_SECTIONS_PER_SLOT \
  (6) /**< Min number of different sections in single slot (section may be equal to RB allocation for UE) */
#define XRAN_MAX_SECTIONS_PER_SYM                                                                                                  \
  (XRAN_MAX_SECTIONS_PER_SLOT) /**< Max number of different sections in single slot (section may be equal to RB allocation for UE) \
                                */
#define XRAN_MIN_SECTIONS_PER_SYM                                                                                                  \
  (XRAN_MIN_SECTIONS_PER_SLOT) /**< Min number of different sections in single slot (section may be equal to RB allocation for UE) \
                                */
...
#define XRAN_VF_QUEUE_MAX \
  (XRAN_MAX_SECTOR_NR * XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_ANT_ARRAY_ELM_NR) /**< MAX number of HW queues for given VF */
...
#ifdef _XRAN_DEBUG
#define xran_log_dbg(fmt, ...) fprintf(stderr, "DEBUG: %s(%d): " fmt "\n", __FILE__, __LINE__, ##__VA_ARGS__)
#else
#define xran_log_dbg(fmt, ...)
#endif

#if defined(_XRAN_DEBUG) || defined(_XRAN_VERBOSE)
#define xran_log_wrn(fmt, ...) fprintf(stderr, "WARNING: %s(%d): " fmt "\n", __FILE__, __LINE__, ##__VA_ARGS__)
#else
#define xran_log_dbg(fmt, ...)
#define xran_log_wrn(fmt, ...)
#endif

#define xran_log_err(fmt, ...) fprintf(stderr, "ERROR: %s(%d): " fmt "\n", __FILE__, __LINE__, ##__VA_ARGS__)
...
enum xran_if_state { XRAN_INIT = 0, XRAN_RUNNING, XRAN_STOPPED, XRAN_OWDM };

typedef enum xran_vMu_proc_type_e { XRAN_VMU_PROC_SSB, XRAN_VMU_PROC_MAX } xran_vMu_proc_type_t;
...
enum xran_memstat_index { XRAN_MEMSTAT_MAXNUM = 0, XRAN_MEMSTAT_AVAIL, XRAN_MEMSTAT_INUSE, XRAN_MEMSTAT_END };
...
struct xran_ecpri_del_meas_port {
  ...
  uint16_t msState; // Measurement State for Initiator: Idle, Waiting_Response, Waiting_Request, Waiting_Request_with_fup,
                    // Waiting_fup, Done Measurement State for Recipient: Idle, Waiting_Response, Waiting_Follow_up, Done
...
struct xran_prach_config {
  ...
  uint16_t timeOffset;
  int32_t freqOffset;
  uint8_t prachEaxcOffset; /* Starting value of Eaxc for PRACH packets for this numerology (Absolute value). Should be unique across
                              all numerologies for the RU */
...
struct xran_srs_config {
  ...
  uint8_t srsEaxcOffset; /**< starting value of eAxC for SRS packets for this numerology (Absolute value). Should be unique across
                            all numerologies for the RU */
};
...
struct xran_slot_config {
  uint8_t
      nSymbolType[XRAN_NUM_OF_SYMBOL_PER_SLOT]; /**< Defines the Symbol type for all 14 symbols in a slot. 0: DL, 1: UL, 2: Guard */
  uint8_t reserved[2];
};
...
typedef struct xran_fh_per_mu_cfg {
  ...
  uint8_t eaxcOffset; /* Starting value of Eaxc for PDSCH, PUSCH packets (Absolute value) of this numerology. Should be unique
                         across all numerologies for the RU */
...
struct xran_fh_config {
  ...
  int32_t GPS_Alpha; // refer to alpha as defined in section 9.7.2 of ORAN spec. this value should be alpha*(1/1.2288ns), range 0 -
                     // 1e7 (ns)
  int32_t GPS_Beta; // beta value as defined in section 9.7.2 of ORAN spec. range -32767 ~ +32767
  ...
  uint16_t tx_cp_eAxC2Vf[XRAN_MAX_SECTOR_NR]
                        [XRAN_MAX_ANTENNA_NR * 2
                         + XRAN_MAX_ANT_ARRAY_ELM_NR]; /**< mapping of C-Plane (ecpriRtcid) or U-Plane (ecpriPcid) to VF */
  uint16_t tx_up_eAxC2Vf[XRAN_MAX_SECTOR_NR]
                        [XRAN_MAX_ANTENNA_NR * 2
                         + XRAN_MAX_ANT_ARRAY_ELM_NR]; /**< mapping of C-Plane (ecpriRtcid) or U-Plane (ecpriPcid) to VF */

  uint16_t rx_cp_eAxC2Vf[XRAN_MAX_SECTOR_NR]
                        [XRAN_MAX_ANTENNA_NR * 2
                         + XRAN_MAX_ANT_ARRAY_ELM_NR]; /**< mapping of C-Plane (ecpriRtcid) or U-Plane (ecpriPcid) to VF */
  uint16_t rx_up_eAxC2Vf[XRAN_MAX_SECTOR_NR]
                        [XRAN_MAX_ANTENNA_NR * 2
                         + XRAN_MAX_ANT_ARRAY_ELM_NR]; /**< mapping of C-Plane (ecpriRtcid) or U-Plane (ecpriPcid) to VF */
...
struct xran_common_counters {
  ...
  /* Error counters */
  uint32_t rx_err_drop; /**< Table 9.1-1 The total number of inbound messages which are discarded by the receiving O-RAN entity for
                           any reason */
 ...
 *   -1 - on failure
 */
int32_t xran_mem_mgr_leak_detector_register_cb_fn(xran_mem_mgr_leak_detector_add_cb_fn pxRANAddFn,
                                                  xran_mem_mgr_leak_detector_remove_cb_fn pxRANRemoveFn);
...
struct rte_mempool *xran_pktmbuf_pool_create(const char *name,
                                             unsigned int n,
 ...
 *   0 - on success
 */
int32_t xran_bm_allocate_ring(void *pHandle,
                              const char *rng_name_prefix,
                              uint16_t cc_id,
                              uint16_t buff_id,
                              uint16_t ant_id,
                              uint16_t symb_id,
...
uint32_t xran_get_time_stats(uint64_t *total_time,
                             uint64_t *used_time,
                             uint32_t *num_core_used,
                             uint32_t *core_used,
                             uint32_t clear);
uint32_t xran_timingsource_get_timestats(uint64_t *total_time,
                                         uint64_t *used_time,
                                         uint32_t *num_core_used,
                                         uint32_t *core_used,
                                         uint32_t clear);
...
int32_t xran_reg_sym_cb(void *pHandle,
                        ...
                        void *symCbParam,
                        struct xran_sense_of_time *symCbTime,
                        uint8_t symb,
                        enum cb_per_sym_type_id cb_sym_t_id,
                        uint8_t mu);
 ...
 *   -1 - in case of failure
 */
int32_t xran_timingsource_reg_tticb(__attribute__((unused)) void *pHandle,
                                    xran_fh_tti_callback_fn Cb,
                                    void *cbParam,
                                    int skipTtiNum,
                                    enum callback_to_phy_id id);
 ...
 */
int32_t xran_get_slot_idx(uint32_t PortId,
                          uint32_t *nFrameIdx,
 ...
 * @ingroup xran
 *
 *   Function prepares UL U-plane, PRACH, SRS packets for symbol for O-RAN FH. Enques resulting packet to ring for TX at appropriate
 * time
 *
 * @param pHandle
 *    pointer to O-RU port structure
