...
#ifdef XRAN_OWD_DEBUG_DELAY_CONV_FUNCTIONS
  printf("PTP ts to ns sec_msb %4" PRIx16 " secs_lsb %4" PRIx32 " ns %4" PRIx32 "  seconds %8" PRIx64 " nanosec %8" PRIx64
         " ret_value %8" PRIx64 "\n",
         t->secs_msb,
         t->secs_lsb,
         t->ns,
...
void xran_initialize_and_verify_owd_pl_length(void *handle)
{
  ...
  if ((p_xran_dev_ctx->eowd_cmn[appMode].owdm_PlLength == 0)
...
void xran_build_owd_meas_ecpri_hdr(char *mbuf, struct xran_ecpri_del_meas_cmn *eowdcmn)
{
  ...
  tmp->bits.ecpri_payl_size = sizeof(struct xran_ecpri_delay_meas_pl)
                              + eowdcmn->owdm_PlLength; // eowdcmn->owdm_PlLength correponds to  the dummy_bytes field which now
                                                        // allows the user to select the length for this field to be sent
...
void xran_initialize_ecpri_del_meas_port(struct xran_ecpri_del_meas_cmn *pCmn,
                                         struct xran_ecpri_del_meas_port *pPort,
                                         uint16_t full)
{
  uint16_t i = 0;
...
int xran_generate_delay_meas(uint16_t port_id, void *handle, uint8_t actionType, uint8_t MeasurementID)
{
  ...
  pkt_len = sizeof(struct xran_ecpri_del_meas_pkt)
            + powdc->owdm_PlLength; // powdc->owdm_PlLength correponds to  the dummy_bytes field which now allows the user to select
                                    // the length for this field to be sent
  ...
  pdm = (struct xran_ecpri_delay_meas_pl *)rte_pktmbuf_mtod_offset(mbuf,
                                                                   struct xran_ecpri_delay_meas_pl *,
                                                                   sizeof(struct rte_ether_hdr) + sizeof(union xran_ecpri_cmn_hdr));
  switch (actionType) {
    ...
    case ECPRI_REQUEST_W_FUP:
      // Record t1, prepare Request with follow up Message and determine tcv1, send zero timestamp and correction value  in the
      // packet 1) Record the current timestamp when the message preparation started i.e. t1
      if (clock_gettime(CLOCK_REALTIME, &tr)) // t1
      {
        return ret_value;
      }
    ...
    case ECPRI_FOLLOW_UP:
      // Use the t1 and tcv1 values recorded in the ECPRI_REQUEST_W_FUP packet generation step and send these values in the follow
      // up packet 1) Prepare the delay measurement follow up packet
  ...
  printf("id is %d\n", appMode);
  printf("Port %u SRC MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
         (unsigned)port_id,
         addr.addr_bytes[0],
         addr.addr_bytes[1],
         addr.addr_bytes[2],
  ...
  if (appMode == O_DU) {
    ...
    printf("DST_MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
           ...
           pa[5]);
#endif
    rte_ether_addr_copy((struct rte_ether_addr *)&p_xran_dev_ctx->p_o_du_addr[0],
                        (struct rte_ether_addr *)&h->dst_addr.addr_bytes[0]);
    ...
    printf("DST_MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
           ...
           pb[5]);
#endif
    rte_ether_addr_copy((struct rte_ether_addr *)&p_xran_dev_ctx->p_o_ru_addr[0],
                        (struct rte_ether_addr *)&h->dst_addr.addr_bytes[0]);
  }
#ifdef XRAN_OWD_DEBUG_PKTS
...
int xran_process_delmeas_request(struct rte_mbuf *pkt, void *handle, struct xran_ecpri_del_meas_pkt *ptr, uint16_t port_id)
{
  ...
  //    pchar = rte_pktmbuf_prepend(pkt, (uint16_t)(sizeof(struct rte_ether_hdr)+ sizeof(union xran_ecpri_cmn_hdr ))); // Pointer to
  //    new data start address 10/20/20 Now not removing ecpri_cmn in process_delay_meas
  /*pchar = */ rte_pktmbuf_prepend(pkt, (uint16_t)sizeof(struct rte_ether_hdr));
  pkt1 = rte_pktmbuf_copy(pkt, _eth_mbuf_pool, 0, UINT32_MAX);
  pdm = (struct xran_ecpri_del_meas_pkt *)rte_pktmbuf_mtod_offset(pkt1,
...
//    printf("EtherType: %04"PRIx16" \n",&h->ether_type);
  ...
  pdm = (struct xran_ecpri_del_meas_pkt *)rte_pktmbuf_mtod_offset(pkt1,
                                                                  struct xran_ecpri_del_meas_pkt *,
                                                                  sizeof(struct rte_ether_hdr));
  pdm->cmnhdr.bits.ecpri_payl_size = sizeof(struct xran_ecpri_delay_meas_pl)
                                     + powdc->owdm_PlLength; // powdc->owdm_PlLength correponds to  the dummy_bytes field which now
                                                             // allows the user to select the length for this field to be sent
  ...
  printf("pdm has:%02" PRIx8 " %04" PRIx16 " %02" PRIx8 " %02" PRIx8 " \n",
         pdm->cmnhdr.bits.ecpri_mesg_type,
         pdm->cmnhdr.bits.ecpri_payl_size,
         pdm->cmnhdr.bits.ecpri_ver,
         pdm->deMeasPl.MeasurementID);
#endif

  // Copy dest address from above
  ret_value =
      rte_eth_tx_burst((uint16_t)*port, 0, &pkt1, 1); // Need to check for the proper method of getting the port and mac address
#ifdef XRAN_OWD_DEBUG_MSG_FLOW
  ...
  printf("Computed delay is %08" PRIx64 " MeasNum %d portId %d id is %d \n",
         powdp->delaySamples[powdp->numMeas],
         powdp->numMeas,
         port_id,
...
int xran_process_delmeas_rem_request(struct rte_mbuf *pkt, void *handle, struct xran_ecpri_del_meas_pkt *ptr, uint16_t port_id)
{
  ...
  // 4) Record the current timestamp at the moment that the delay measurement packet is ready to be transmitted tr2 i.e.t1+tcv1 and
  // write it
  //    to the Delay Measurement request packet PL field
...
int xran_process_delmeas_rem_request_w_fup(struct rte_mbuf *pkt,
                                           void *handle,
                                           struct xran_ecpri_del_meas_pkt *ptr,
                                           uint16_t port_id)
{
  int ret_value = FAIL;
  int appMode = xran_get_syscfg_appmode();
  ...
  pdm = (struct xran_ecpri_del_meas_pkt *)rte_pktmbuf_mtod_offset(pkt2,
                                                                  struct xran_ecpri_del_meas_pkt *,
                                                                  sizeof(struct rte_ether_hdr));
...
#endif
  ret_value =
      rte_eth_tx_burst((uint16_t)*port, 0, &pkt2, 1); // Need to check for the proper method of getting the port and mac address
