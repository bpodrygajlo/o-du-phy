...
struct rte_mbuf *xran_attach_up_ext_buf(uint16_t vf_id,
                                        int8_t *p_ext_buff_start,
                                        int8_t *p_ext_buff,
...
int32_t xran_process_tx_sym_cp_off(void *pHandle,
                                   uint8_t ctx_id,
                                   uint32_t tti,
                                   int32_t cc_id,
                                   int32_t ant_id,
                                   ...
                                   uint32_t sym_id_for_ring)
{
  ...
  if (xran_fs_get_slot_type(PortId, cc_id, tti, ((appMode == O_DU) ? XRAN_SLOT_TYPE_DL : XRAN_SLOT_TYPE_UL), mu) == 1
    ...
    if (xran_fs_get_symbol_type(PortId, cc_id, tti, sym_id, mu) == ((appMode == O_DU) ? XRAN_SYMBOL_TYPE_DL : XRAN_SYMBOL_TYPE_UL)
      ...
      pos = (char *)p_xran_dev_ctx->perMu[mu]
                .sFrontHaulTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                .sBufferList.pBuffers[sym_id]
                .pData;
      mb = (void *)p_xran_dev_ctx->perMu[mu]
               .sFrontHaulTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
               .sBufferList.pBuffers[sym_id]
               .pCtrl;
      prb_map = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                    .sFrontHaulTxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                    .sBufferList.pBuffers->pData;
      ring = p_xran_dev_ctx->perMu[mu]
                 .sFrontHaulTxBbuIoBufCtrl[tti_for_ring % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                 .sBufferList.pBuffers[sym_id_for_ring]
                 .pRing;
      ...
      if (prb_map) {
        ...
        for (elmIdx = 0; elmIdx < prb_map->nPrbElm && elmIdx < XRAN_MAX_SECTIONS_PER_SLOT; elmIdx++) {
          // print_err("tti is %d, cc_id is %d, ant_id is %d, prb_map->nPrbElm id - %d", tti % XRAN_N_FE_BUF_LEN, cc_id, ant_id,
          // prb_map->nPrbElm);
          ...
          if (appMode == O_RU && xran_get_ru_category(pHandle) == XRAN_CATEGORY_B) {
          ...
          if (NULL == send_mb) {
            ...
            ext_buff = p_sec_iq
                       - (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr)
                          + sizeof(struct data_section_hdr));

            ext_buff_len += RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr)
            ...
            seq_id =
                (appMode == O_DU) ? xran_get_updl_seqid(PortId, cc_id, ru_port_id) : xran_get_upul_seqid(PortId, cc_id, ru_port_id);
            ...
            prepare_symbol_ex(direction,
                              ...
                              slot_id,
                              sym_id,
                              prb_map_elm->UP_nRBStart,
                              prb_map_elm->UP_nRBSize,
                              cc_id,
                              ru_port_id,
                              seq_id,
                              0,
            ...
            /* first all PRBs */
            prepare_symbol_ex_appand(direction,
                                     curr_sect_id,
                                     send_mb,
                                     (uint8_t *)p_sec_iq,
                                     prb_map_elm->compMethod,
...
int32_t xran_process_tx_prach_cp_off(void *pHandle,
                                     uint8_t ctx_id,
                                     uint32_t tti,
                                     int32_t cc_id,
                                     int32_t ant_id,
                                     ...
                                     uint32_t sym_id_for_ring)
{
  ...
  if (p_xran_dev_ctx->perMu[mu].enablePrach && (xran_get_syscfg_appmode() == O_RU) && (ant_id < XRAN_MAX_PRACH_ANT_NUM)) {
    if (xran_fs_get_symbol_type(PortId, cc_id, tti, sym_id, mu) == XRAN_SYMBOL_TYPE_UL
      ...
      if (mu != XRAN_NBIOT_MU) {
        ...
        if (((frame_id % pPrachCPConfig->x) == pPrachCPConfig->y[0]) && (is_prach_slot == 1)
            && (sym_id >= p_xran_dev_ctx->prach_start_symbol[cc_id]) && (sym_id <= p_xran_dev_ctx->prach_last_symbol[cc_id])) {
          sendPrach = 1;
        } else
          sendPrach = 0;
      } else {
      ...
      if (sendPrach) {
        ...
        pos = (char *)p_xran_dev_ctx->perMu[mu]
                  .sFHPrachRxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                  .sBufferList.pBuffers[symb_id_offset]
                  .pData;
        ...
        mb = NULL; //(void*) p_xran_dev_ctx->perMu[mu].sFHPrachRxBbuIoBufCtrl[tti %
                   //XRAN_N_FE_BUF_LEN][cc_id][ant_id].sBufferList.pBuffers[0].pCtrl;
        vf_id = xran_map_ecpriPcid_to_vf(p_xran_dev_ctx, direction, cc_id, prach_port_id);
        pRing = p_xran_dev_ctx->perMu[mu]
                    .sFrontHaulTxBbuIoBufCtrl[tti_for_ring % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                    .sBufferList.pBuffers[sym_id_for_ring]
                    .pRing;
        ...
        if (1500 == p_xran_dev_ctx->mtu && pPrachCPConfig->filterIdx == XRAN_FILTERINDEX_PRACH_012) {
          ...
          send_symbol_ex(pHandle,
                         ...
                         cc_id,
                         prach_port_id,
                         xran_get_upul_seqid(PortId, cc_id, prach_port_id),
                         mu);
        }
        retval = 1;
      }
...
int32_t xran_process_tx_srs_cp_off(void *pHandle,
                                   uint8_t ctx_id,
                                   uint32_t tti,
                                   int32_t cc_id,
                                   int32_t ant_id,
                                   ...
                                   uint32_t slot_id)
{
  ...
  prb_map = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                .sFHSrsRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                .sBufferList.pBuffers->pData;
  if (prb_map) {
    for (elmIdx = 0; elmIdx < prb_map->nPrbElm && elmIdx < XRAN_MAX_SECTIONS_PER_SLOT; elmIdx++) {
      struct xran_prb_elm *prb_map_elm = &prb_map->prbMap[elmIdx];
      ...
      pos = (char *)p_xran_dev_ctx->perMu[mu]
                .sFHSrsRxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                .sBufferList.pBuffers[sym_id]
                .pData;
      mb = (void *)p_xran_dev_ctx->perMu[mu]
               .sFHSrsRxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
      ...
      rte_pktmbuf_attach_extbuf(eth_oran_hdr, ext_buff, ext_buff_iova + RTE_PTR_DIFF(ext_buff, mb), ext_buff_len, p_share_data);
...
int32_t xran_process_tx_sym_cp_on_dispatch_opt(void *pHandle,
                                               uint8_t ctx_id,
                                               uint32_t tti,
                                               int32_t start_cc,
                                               int32_t num_cc,
...
int32_t do_nothing(void *pHandle,
                   uint8_t ctx_id,
                   uint32_t tti,
                   int32_t start_cc,
                   int32_t num_cc,
...
int32_t xran_enqueue_timing_info_for_tx(void *pHandle,
                                        uint8_t ctx_id,
                                        uint32_t tti,
                                        int32_t start_cc,
                                        int32_t num_cc,
...
int32_t xran_prepare_up_dl_sym(uint16_t xran_port_id,
                               uint32_t nSlotIdx,
                               uint32_t nCcStart,
                               uint32_t nCcNum,
                               uint32_t nSymMask,
  ...
  for (idxSym = nSymStart; idxSym < (nSymStart + nSymNum) && idxSym < XRAN_NUM_OF_SYMBOL_PER_SLOT; idxSym++) {
    ...
    if (((1 << idxSym) & nSymMask)) {
      ...
      if ((sym_idx_to_send + p_xran_dev_ctx->perMu[mu].sym_up) < 0) {
        ...
        sym_idx_for_ring =
            XRAN_NUM_OF_SYMBOL_PER_SLOT * SLOTNUM_PER_SUBFRAME(interval) * 1000 + idxSym + p_xran_dev_ctx->perMu[mu].sym_up;
      } else {
        sym_idx_for_ring = sym_idx_to_send + p_xran_dev_ctx->perMu[mu].sym_up;
      }

      sym_idx_to_send =
          XranOffsetSym(0, sym_idx_to_send, XRAN_NUM_OF_SYMBOL_PER_SLOT * SLOTNUM_PER_SUBFRAME(interval) * 1000, &inPeriod);
      ...
      if (appMode == O_DU && p_xran_dev_ctx->enableCP) {
        ...
        if (loc_ret) {
          retval = xran_process_tx_sym_cp_on_opt(pHandle,
                                                 ctx_id,
                                                 tti,
                                                 nCcStart,
                                                 nCcNum,
                                                 (nAntStart + p_xran_dev_ctx->perMu[mu].eaxcOffset),
                                                 nAntNum,
                                                 frame_id,
                                                 subframe_id,
                                                 slot_id,
        ...
        if (xran_get_ru_category(pHandle) == XRAN_CATEGORY_B && p_xran_dev_ctx->csirsEnable && nAntStart == 0) {
          retval = xran_process_tx_csirs_cp_on(pHandle,
                                               ctx_id,
                                               tti,
                                               nCcStart,
                                               nCcNum,
                                               0 /*pCsirsCfg->csirsEaxcOffset*/,
                                               XRAN_MAX_CSIRS_PORTS,
                                               frame_id,
                                               subframe_id,
                                               slot_id,
                                               sym_id,
          ...
          if (mu == 3 && p_xran_dev_ctx->vMuInfo.ssbInfo.ssbMu == 4) {
            ...
            if (likely(loc_ret)) {
              for (uint8_t m = 0; m < NumSsbSymPerTimerSym; ++m) {
                ...
                uint32_t ssbSfId =
                    (ssbSlotId / SLOTNUM_PER_SUBFRAME_MU(p_xran_dev_ctx->vMuInfo.ssbInfo.ssbMu)) % SUBFRAMES_PER_SYSTEMFRAME;

                retval = xran_process_tx_sym_cp_on_opt(pHandle,
                                                       (ssbTti % XRAN_MAX_SECTIONDB_CTX),
                                                       ssbTti,
                                                       nCcStart,
                                                       nCcNum,
                                                       p_xran_dev_ctx->vMuInfo.ssbInfo.ruPortId_offset,
                                                       nAntNum,
                                                       frame_id,
                                                       ssbSfId,
                                                       ssbSlotId,
          ...
          for (cc_id = 0; cc_id < num_CCPorts; cc_id++) {
            ...
            if (p_xran_dev_ctx->puschMaskEnable) {
              if ((tti % numSlotMu1) != p_xran_dev_ctx->puschMaskSlot)
                retval = xran_process_tx_sym_cp_off(pHandle,
                                                    ctx_id,
                                                    tti,
                                                    cc_id,
                                                    ant_id,
            ...
            if (p_xran_dev_ctx->perMu[mu].enablePrach && (ant_id < num_eAxc_prach)) {
              retval = xran_process_tx_prach_cp_off(pHandle,
                                                    ctx_id,
                                                    tti,
                                                    cc_id,
                                                    ant_id,
      ...
      if (appMode == O_RU && xran_get_ru_category(pHandle) == XRAN_CATEGORY_B && p_xran_dev_ctx->enableSrs
        ...
        for (cc_id = 0; cc_id < num_CCPorts; cc_id++) {
          ...
          if ((xran_fs_get_slot_type(portId, cc_id, tti, XRAN_SLOT_TYPE_SP, mu) == 1)
            ...
            if (((tti % p_xran_dev_ctx->fh_cfg.frame_conf.nTddPeriod) == pSrsCfg->slot)
              ...
              prb_map = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                            .sFHSrsRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][0]
                            .sBufferList.pBuffers->pData;
          ...
          else if (p_xran_dev_ctx->ndm_srs_scheduled && p_xran_dev_ctx->ndm_srs_txtti == tti) {
            int ndm_step;
...
static inline uint16_t xran_tx_sym_from_ring(struct xran_device_ctx *p_xran_dev_ctx, struct rte_ring *r, uint16_t vf_id)
{
  ...
  dequeued = rte_ring_dequeue_burst(r, (void **)mbufs, XRAN_MAX_MEM_IF_RING_SIZE, &remaining);
  if (!dequeued)
...
int32_t xran_process_tx_sym_cp_on_ring(void *pHandle,
                                       uint8_t ctx_id,
                                       uint32_t tti,
                                       int32_t start_cc,
                                       int32_t num_cc,
...
int32_t xran_process_tx_sym_cp_on_ring_opt(void *pHandle,
                                           int32_t start_cc,
                                           int32_t num_cc,
                                           int32_t start_ant,
                                           int32_t num_ant,
...
{tti, symbol} specific rings are created at initialization. Caller of this function should figure out the exact {tti, sym} to start
sending these packets and provide them here.
*/
int32_t xran_process_tx_sym_cp_on_opt(void *pHandle,
                                      uint8_t ctx_id,
                                      uint32_t tti,
                                      int32_t start_cc,
                                      int32_t num_cc,
                                      ...
                                      bool isVmu)
{
  ...
  const uint8_t total_header_size = (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr)
                                     + sizeof(struct data_section_hdr));
  ...
  for (cc_id = start_cc; cc_id < (start_cc + num_cc); cc_id++) {
    ...
    for (ant_id = start_ant; ant_id < (start_ant + num_ant); ant_id++) {
      ...
      if (0 != ptr_sect_elm->cur_index) {
        ...
        p_id = eth_ctx->io_cfg.port[vf_id];
        ring = p_xran_dev_ctx->perMu[mu]
                   .sFrontHaulTxBbuIoBufCtrl[tti_for_ring % XRAN_N_FE_BUF_LEN][cc_id][ant_index]
                   .sBufferList.pBuffers[sym_id_for_ring]
                   .pRing;
        if (!isVmu) {
          mb_base = p_xran_dev_ctx->perMu[mu]
                        .sFrontHaulTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_index]
                        .sBufferList.pBuffers[sym_id]
                        .pCtrl;
          temp_buff = ((char *)p_xran_dev_ctx->perMu[mu]
                           .sFrontHaulTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_index]
                           .sBufferList.pBuffers[sym_id]
                           .pData);
        } else {
          if (vMu_proc_type == XRAN_VMU_PROC_SSB) {
            mb_base = p_xran_dev_ctx->vMuInfo.ssbInfo.sFHSsbTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_index]
                          .sBufferList.pBuffers[sym_id]
                          .pCtrl;
            temp_buff = (char *)p_xran_dev_ctx->vMuInfo.ssbInfo.sFHSsbTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_index]
                            .sBufferList.pBuffers[sym_id]
                            .pData;
          }
        }
        ...
        cid = ((cc_id << p_xran_dev_ctx->eAxc_id_cfg.bit_ccId) & p_xran_dev_ctx->eAxc_id_cfg.mask_ccId)
              | ((ant_id << p_xran_dev_ctx->eAxc_id_cfg.bit_ruPortId) & p_xran_dev_ctx->eAxc_id_cfg.mask_ruPortId);
...
#pragma loop_count min = 1, max = 16 // XRAN_MAX_SECTIONS_PER_SYM
        for (next = 0; next < num_sections; next++) {
          ...
          if (unlikely(sectinfo->type != XRAN_CP_SECTIONTYPE_1
                       && sectinfo->type != XRAN_CP_SECTIONTYPE_3)) { /* only supports type 1, 3 */
          ...
          if (sectinfo->prbElemBegin || p_xran_dev_ctx->RunSlotPrbMapBySymbolEnable) {
            ...
            iq_sample_size_bytes = 18 + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr);
          }
          ...
          if (sectinfo->prbElemBegin || p_xran_dev_ctx->RunSlotPrbMapBySymbolEnable) {
            ...
            if (unlikely(((rte_iova_t)(struct rte_mempool_objhdr *)RTE_PTR_SUB(mb_base, rte_mempool_objhdr_size)->iova)
                         == RTE_BAD_IOVA)) {
              rte_panic("Failed rte_mem_virt2iova RTE_BAD_IOVA \n");
            }
#endif
            ...
            mb_oran_hdr_ext->buf_iova = ((struct rte_mempool_objhdr *)RTE_PTR_SUB(mb_base, rte_mempool_objhdr_size))->iova
                                        + RTE_PTR_DIFF(ext_buff, mb_base);
            mb_oran_hdr_ext->buf_len = ext_buff_len;
            mb_oran_hdr_ext->ol_flags |= RTE_MBUF_F_EXTERNAL;
            mb_oran_hdr_ext->shinfo = p_share_data;
            mb_oran_hdr_ext->data_off =
                (uint16_t)RTE_MIN((uint16_t)RTE_PKTMBUF_HEADROOM, (uint16_t)mb_oran_hdr_ext->buf_len) - rte_ether_hdr_size;
            ...
            p_xran_dev_ctx->perMu[mu].to_free_mbuf[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id][sym_id][section_id] =
                (void *)mb_oran_hdr_ext;
...
#endif
            nPktSize = sizeof(struct rte_ether_hdr) + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr);
            ...
            ecpri_hdr->cmnhdr.bits.ecpri_payl_size =
                sizeof(struct radio_app_common_hdr) + XRAN_ECPRI_HDR_SZ; // xran_get_ecpri_hdr_size();;;
              ...
              if (xran_get_syscfg_bbuoffload()) {
                ...
                if (true == xran_check_if_late_transmission(tti_for_ring % XRAN_N_FE_BUF_LEN, sym_id_for_ring, mu)) {
                  print_err("\nxran_port_id=%u, uplane too late:ttiSymInPkt={%u, %u, %u}, ttiSymToSend={%u,%u} \n\n",
                            xran_port_id,
                            tti,
                            tti % XRAN_N_FE_BUF_LEN,
                            sym_id,
                            tti_for_ring % XRAN_N_FE_BUF_LEN,
                            sym_id_for_ring);
                  rte_panic("\n");
                }
#endif
...
int32_t xran_process_tx_srs_cp_on(void *pHandle,
                                  uint8_t ctx_id,
                                  uint32_t tti,
                                  int32_t start_cc,
                                  int32_t num_cc,
  ...
  for (cc_id = start_cc; cc_id < (start_cc + num_cc); cc_id++) {
    ...
    for (ant_id = start_ant; ant_id < (start_ant + num_ant); ant_id++) {
      ...
      if (0 != ptr_sect_elm->cur_index) {
        num_sections = ptr_sect_elm->cur_index;
        /* iterate C-Plane configuration to generate corresponding U-Plane */
        vf_id = xran_map_ecpriPcid_to_vf(p_xran_dev_ctx,
                                         direction,
                                         cc_id,
                                         antElm_eAxC_id); // p_xran_dev_ctx->map2vf[direction][cc_id][antElm_eAxC_id][XRAN_UP_VF];
        mb_base = p_xran_dev_ctx->perMu[mu]
                      .sFHSrsRxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                      .sBufferList.pBuffers[sym_id]
                      .pCtrl;
        ...
        cid = ((cc_id << p_xran_dev_ctx->eAxc_id_cfg.bit_ccId) & p_xran_dev_ctx->eAxc_id_cfg.mask_ccId)
              | ((antElm_eAxC_id << p_xran_dev_ctx->eAxc_id_cfg.bit_ruPortId) & p_xran_dev_ctx->eAxc_id_cfg.mask_ruPortId);
        cid = rte_cpu_to_be_16(cid);
#pragma loop_count min = 1, max = 16
        for (next = 0; next < num_sections; next++) {
          ...
          if (unlikely(sectinfo->type != XRAN_CP_SECTIONTYPE_1
                       && sectinfo->type != XRAN_CP_SECTIONTYPE_3)) { /* only supports type 1 or type 3*/
            print_err("Invalid section type in section DB - %d", sectinfo->type);
            continue;
          }
          /* skip, if not scheduled */
          ...
          if (sectinfo->prbElemBegin) {
            ...
            ext_buff = ((char *)p_xran_dev_ctx->perMu[mu]
                            .sFHSrsRxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                            .sBufferList.pBuffers[sym_id]
                            .pData
                        + sectinfo->sec_desc[sym_id].iq_buffer_offset)
                       - total_header_size;
            ext_buff_len += (total_header_size + 18);
...
int32_t xran_process_tx_csirs_cp_on(void *pHandle,
                                    uint8_t ctx_id,
                                    uint32_t tti,
                                    int32_t start_cc,
                                    int32_t num_cc,
  ...
  for (cc_id = start_cc; cc_id < (start_cc + num_cc); cc_id++) {
    ...
    for (ant_id = start_ant; ant_id < (start_ant + num_ant); ant_id++) {
      ...
      if (p_xran_dev_ctx->perMu[mu].sFHCsirsTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id].sBufferList.pBuffers)
        mb_base = p_xran_dev_ctx->perMu[mu]
                      .sFHCsirsTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
                      .sBufferList.pBuffers[sym_id]
                      .pCtrl;
      else
        continue;
      ...
      if (0 != ptr_sect_elm->cur_index) {
        ...
        vf_id = xran_map_ecpriPcid_to_vf(p_xran_dev_ctx,
                                         ...
                                         antElm_eAxC_id); // p_xran_dev_ctx->map2vf[direction][cc_id][antElm_eAxC_id][XRAN_UP_VF];
        ring = p_xran_dev_ctx->perMu[mu]
                   .sFrontHaulTxBbuIoBufCtrl[tti_for_ring % XRAN_N_FE_BUF_LEN][cc_id % num_CCPorts][ant_id % num_eAxc]
                   .sBufferList.pBuffers[sym_id_for_ring]
                   .pRing;
        temp_buff = ((char *)p_xran_dev_ctx->perMu[mu]
                         .sFHCsirsTxBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][ant_id]
...
#pragma loop_count min = 1, max = 16
        for (next = 0; next < num_sections; next++) {
          ...
          if (sectinfo->prbElemBegin) {
            ...
            // ext_buff = ((char*)p_xran_dev_ctx->perMu[mu].sFHCsirsTxBbuIoBufCtrl[tti %
            // XRAN_N_FE_BUF_LEN][cc_id][ant_id].sBufferList.pBuffers[sym_id].pData + sectinfo->sec_desc[sym_id].iq_buffer_offset) -
            // total_header_size;
          ...
          if (sectinfo->prbElemEnd) /* Transmit the packet */
          {
            ...
            if (xran_get_syscfg_bbuoffload()) {
              ...
              if (likely(p_xran_dev_ctx->mtu >= (iq_sample_size_bytes))) {
                ...
                if (ret == 0)
                  print_err("vf_id = %d sectinfo->startSymId = %d sectinfo->numSymbol = %d",
                            vf_id,
                            sectinfo->startSymId,
                            sectinfo->numSymbol);
...
int32_t xran_process_tx_sym_oru(void *arg, uint8_t mu)
{
  ...
  if (xran_get_syscfg_bbuoffload()) {
    sym_idx = XranOffsetSym(0,
                            xran_lib_ota_sym_idx_mu[mu],
                            XRAN_NUM_OF_SYMBOL_PER_SLOT * SLOTNUM_PER_SUBFRAME(interval) * 1000,
                            &inPeriod);
  } else {
    sym_idx = XranOffsetSym(p_xran_dev_ctx->perMu[mu].sym_up,
                            xran_lib_ota_sym_idx_mu[mu],
                            XRAN_NUM_OF_SYMBOL_PER_SLOT * SLOTNUM_PER_SUBFRAME(interval) * 1000,
                            &inPeriod);
  }
  ...
  //    if(p_xran_dev_ctx->fh_init.io_cfg.id == O_RU) // this functions is called only with RU mode
  {
    if ((XRAN_NBIOT_MU == mu)
        && (XRAN_NBIOT_UL_SCS_3_75
            == p_xran_dev_ctx->fh_cfg.perMu[mu]
                   .nbIotUlScs)) { /* for NB-IOT 3.75 UL: RU will only transmit on specific symbols
      ...
      if (p_xran_dev_ctx->enableCP) {
        ...
        if (xran_fs_get_slot_type(portId, cc_id, tti, XRAN_SLOT_TYPE_UL, mu) == 1
          ...
          if (xran_fs_get_symbol_type(portId, cc_id, tti, sym_id, mu) == XRAN_SYMBOL_TYPE_UL
            ...
            if (loc_ret) {
              xran_process_tx_sym_cp_on_opt(pHandle,
                                            ctx_id,
                                            tti,
                                            0,
                                            num_CCPorts,
        ...
        if (xran_get_ru_category(pHandle) == XRAN_CATEGORY_B && p_xran_dev_ctx->enableSrs) {
          p_sec_db = p_sectiondb[portId][mu];
          compType = p_xran_dev_ctx->fh_cfg.ru_conf.xranCompHdrType;
          struct xran_srs_config *pSrsCfg = &(p_xran_dev_ctx->srs_cfg);
          struct xran_prb_map *prb_map;
          /* check SRS NDM UP has been scheduled in slots */
          if (p_xran_dev_ctx->ndm_srs_scheduled && p_xran_dev_ctx->ndm_srs_txtti == tti) {
            prb_map = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                          .sFHSrsRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][cc_id][0]
                          .sBufferList.pBuffers->pData;
            p_sec_db = p_sectiondb[portId][mu];
            ...
            if (prb_map && prb_map->nPrbElm) {
              ...
              if (sym_id < pSrsCfg->ndm_txduration) {
                retval = xran_process_tx_srs_cp_on(pHandle,
                                                   srsCtx,
                                                   srs_tti,
                                                   0,
                                                   num_CCPorts,
            ...
            if (p_xran_dev_ctx->puschMaskEnable) {
              if ((tti % numSlotMu1) != p_xran_dev_ctx->puschMaskSlot) {
                if (p_xran_dev_ctx->tx_sym_gen_func && p_xran_dev_ctx->use_tx_sym_gen_func) {
                  p_xran_dev_ctx->tx_sym_gen_func(pHandle,
                                                  ctx_id,
                                                  tti,
                                                  cc_id,
                                                  1,
                                                  ...
                                                  slot_id,
                                                  sym_id,
                                                  p_xran_dev_ctx->fh_cfg.ru_conf.xranCompHdrType,
                                                  XRAN_DIR_UL,
                                                  p_xran_dev_ctx->xran_port_id,
                                                  NULL,
                                                  mu,
...
int32_t xran_process_tx_sym(void *arg, uint8_t mu)
{
  ...
  if (xran_get_syscfg_bbuoffload()) {
    sym_idx = XranOffsetSym(0,
                            pCtx->ebbu_offload_ota_sym_cnt_mu[mu],
                            XRAN_NUM_OF_SYMBOL_PER_SLOT * SLOTNUM_PER_SUBFRAME(interval) * 1000,
                            &inPeriod);
  } else {
    sym_idx = XranOffsetSym(p_xran_dev_ctx->perMu[mu].sym_up,
  ...
  if (appMode == O_DU && p_xran_dev_ctx->enableCP) {
    if (p_xran_dev_ctx->tx_sym_gen_func) {
      ...
      if (p_xran_dev_ctx->csirsEnable && xran_get_syscfg_bbuoffload() == 0 && xran_get_ru_category(pHandle) == XRAN_CATEGORY_B) {
        retval = xran_process_tx_csirs_cp_on(pHandle,
                                             ctx_id,
                                             tti,
                                             0,
      ...
      if (p_xran_dev_ctx->vMuInfo.ssbInfo.ssbMu > mu && xran_get_ru_category(pHandle) == XRAN_CATEGORY_A
        ...
        if ((p_xran_dev_ctx->vMuInfo.ssbInfo.ssbMu == 4) && (mu == 3)) {
          ...
          for (k = 0; k < numSsbSymPerTimerMuSym; ++k) {
            ...
            if (unlikely(xran_process_tx_sym_cp_on_opt(pHandle,
                                                       (ssbTti % XRAN_MAX_SECTIONDB_CTX),
                                                       ssbTti,
                                                       0,
                                                       num_CCPorts,
...
int32_t xran_prepare_up_ul_tx_sym(uint16_t xran_port_id,
                                  uint32_t nSlotIdx,
                                  uint32_t nCcStart,
                                  uint32_t nCcNum,
                                  uint32_t nSymMask,
                                  ...
                                  uint8_t mu)
{
  ...
  for (idxSym = nSymStart; idxSym < (nSymStart + nSymNum) && idxSym < XRAN_NUM_OF_SYMBOL_PER_SLOT; idxSym++) {
    ...
    if (((1 << idxSym) & nSymMask)) {
      ...
      /* We are assuming that we will send in the n-1 slot wrt ota here */
      sym_idx_for_ring =
          sym_idx_to_send + p_xran_dev_ctx->perMu[mu].sym_up; /*XranOffsetSym(p_xran_dev_ctx->perMu[mu].sym_up, sym_idx_to_send -
            XRAN_NUM_OF_SYMBOL_PER_SLOT, XRAN_NUM_OF_SYMBOL_PER_SLOT*SLOTNUM_PER_SUBFRAME(interval)*1000, &inPeriod);*/
      ...
      if (first_call && p_xran_dev_ctx->enableCP) { /* U-Plane, SRS Uplink CP-enable=1 (PRACH disabled)*/
        ...
        for (ant_id = nAntStart; ant_id < (nAntStart + nAntNum); ant_id++) {
          for (cc_id = nCcStart; cc_id < (nCcStart + nCcNum); cc_id++) {
            ...
            // It should be as in the initialization i.e ant_id < Nrx_antennas // Nrx_antennas = RTE_MIN(pXranConf->neAxcUl,
            // XRAN_MAX_PRACH_ANT_NUM);
            if (p_xran_dev_ctx->perMu[mu].enablePrach && (ant_id < num_eAxc_prach)) {
              retval = xran_process_tx_prach_cp_off(pHandle,
                                                    ctx_id,
                                                    tti,
