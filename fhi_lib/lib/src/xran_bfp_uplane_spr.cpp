...
void applyExpansionN_1RB(const BlockFloatCompander::CompressedData& dataIn,
                         ...
                         float fScale)
{
  __m512h ph_xmmX, fpscale = _mm512_set1_ph((_Float16)1 / fScale);
  ...
  ph_xmmX = _mm512_cvtepi16_ph(dataExpanded);
  ph_xmmX = _mm512_mul_ph(ph_xmmX, fpscale);
  /// Write expanded data to output
  static constexpr uint32_t k_WriteMask = 0x00FFFFFF;
  _mm512_mask_storeu_epi16(dataOut->dataExpanded + thisRBAddr, k_WriteMask, ph_xmmX);
}
...
#pragma unroll(16)
      for (int n = 0; n < 16; ++n) {
        applyExpansionN_1RB<networkByteUnpack>(dataIn, dataOut, n * totNumBytesPerRB, n * k_numREReal, maxExpShift, fScale);
      }
      break;
...
#pragma unroll(4)
    ...
    case 1:
      applyExpansionN_1RB<networkByteUnpack>(dataIn, dataOut, 0, 0, maxExpShift, fScale);
      break;
  }
}
                         ...
                         float fScale)
{
  ...
  ph_xmmX = _mm512_cvtepi16_ph(expData);
  ph_xmmX = _mm512_mul_ph(ph_xmmX, fpscale);
  constexpr uint8_t k_rbMask64 = 0b00111111; // 64b write mask for 1RB (24 int16 values)
  _mm512_mask_storeu_epi64(dataOut->dataExpanded + thisRBAddr, k_rbMask64, ph_xmmX);
}

/// Calls expansion function specific to the number of RB to be executed. For 8 bit iqWidth.
void expandByAlloc8(const BlockFloatCompander::CompressedData& dataIn, BlockFloatCompander::ExpandedData* dataOut, float fScale)
{
  switch (dataIn.numBlocks) {
    case 16:
#pragma unroll(16)
      for (int n = 0; n < 16; ++n) {
        applyExpansion8_1RB(dataIn, dataOut, n * (k_numREReal + 1), n * k_numREReal, fScale);
      }
      break;
...
#pragma unroll(4)
    ...
    case 1:
      applyExpansion8_1RB(dataIn, dataOut, 0, 0, fScale);
      break;
  }
}
} // namespace BFP_UPlane_SPR
...
void BlockFloatCompander::BFPExpandUserPlaneSpr(const CompressedData& dataIn, ExpandedData* dataOut, float fScale)
{
  ...
  switch (dataIn.iqWidth) {
    case 8:
      BFP_UPlane_SPR::expandByAlloc8(dataIn, dataOut, fScale);
      break;

    case 9:
      BFP_UPlane_SPR::expandByAllocN<BlockFloatCompander::networkByteUnpack9bSnc>(dataIn,
                                                                                  dataOut,
                                                                                  k_totNumBytesPerRB9,
                                                                                  k_maxExpShift9,
                                                                                  fScale);
    ...
    case 10:
      BFP_UPlane_SPR::expandByAllocN<BlockFloatCompander::networkByteUnpack10bSnc>(dataIn,
                                                                                   dataOut,
    ...
    case 12:
      BFP_UPlane_SPR::expandByAllocN<BlockFloatCompander::networkByteUnpack12bSnc>(dataIn,
                                                                                   dataOut,
