...
uint8_t xran_cp_seq_id_num[XRAN_PORTS_NUM][XRAN_MAX_CELLS_PER_PORT][XRAN_DIR_MAX]
                          [XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_ANT_ARRAY_ELM_NR]; /* XRAN_MAX_ANTENNA_NR * 2 for PUSCH and PRACH */
rte_atomic16_t xran_updl_seq_id_num[XRAN_PORTS_NUM][XRAN_MAX_CELLS_PER_PORT][XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_CSIRS_PORTS];
rte_atomic16_t xran_upul_seq_id_num[XRAN_PORTS_NUM][XRAN_MAX_CELLS_PER_PORT]
                                   [XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_ANT_ARRAY_ELM_NR]; /**< PUSCH, PRACH, SRS for Cat B */
uint8_t xran_section_id_curslot[XRAN_PORTS_NUM][XRAN_DIR_MAX][XRAN_MAX_CELLS_PER_PORT]
                               [XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_ANT_ARRAY_ELM_NR];
uint16_t xran_section_id[XRAN_PORTS_NUM][XRAN_DIR_MAX][XRAN_MAX_CELLS_PER_PORT]
                        [XRAN_MAX_ANTENNA_NR * 2 + XRAN_MAX_ANT_ARRAY_ELM_NR];
...
extern __rte_always_inline int8_t
xran_check_updl_seqid(int8_t xran_port, uint8_t cc_id, uint8_t ant_id, uint8_t slot_id, uint8_t seq_id);
extern __rte_always_inline int8_t
xran_check_upul_seqid(int8_t xran_port, uint8_t cc_id, uint8_t ant_id, uint8_t slot_id, uint8_t seq_id);
...
int32_t process_cplane(struct rte_mbuf *pkt, void *handle)
{
  ...
  if (p_xran_dev_ctx && xran_if_current_state == XRAN_RUNNING) {
    if (xran_dev_get_ctx_by_id(0)->fh_cfg.debugStop) /* check CP with standard tests only */
      ret = xran_parse_cp_pkt(pkt,
                              &recvCpInfo[p_xran_dev_ctx->xran_port_id],
                              &parse_recv[p_xran_dev_ctx->xran_port_id],
                              (void *)p_xran_dev_ctx,
                              &mb_free);
...
int32_t xran_check_symbolrange(int symbol_type,
                               uint32_t xranPortId,
                               int ccId,
                               int tti,
                               int start_sym,
...
struct rte_mbuf *xran_attach_cp_ext_buf(int8_t *p_ext_buff_start /*ext_start*/,
                                        int8_t *p_ext_buff /*ext-section*/,
                                        ...
                                        struct rte_mbuf_ext_shared_info *p_share_data)
{
  ...
  ext_buff = p_ext_buff - (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct xran_cp_radioapp_section1_header));

  ext_buff_len += (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct xran_cp_radioapp_section1_header) + 18);
...
static inline void
get_f_sf_slot(uint8_t mu, uint32_t tti, bool isNb375, uint32_t *frameId, uint32_t *sfId, uint32_t *slotId, uint8_t bbuoffload)
{
...
static inline int get_prb_elm_to_process(struct xran_device_ctx *pDevCtx,
                                         ...
                                         uint32_t *i)
{
  ...
  *i = 0;
  if (XRAN_DIR_DL == dir && 0 == pDevCtx->dlCpProcBurst && pDevCtx->DynamicSectionEna == 0) {
    ...
    if (0 == prbElmProcInfo->numSymsRemaining) { /* new slot */
    ...
    if (1 == prbElmProcInfo->numSymsRemaining) { /* last symbol:: send all remaining */
      *numPrbElm = prbMap->nPrbElm;
    } else {
...
static inline void populate_ext1_info(uint16_t curSectId,
                                      int next,
...
static inline void populate_ext4_info(int next, struct xran_sectionext4_info *ext4, struct xran_prb_elm *pPrbElm)
{
...
static inline void populate_ext11_info(int next,
                                       xran_pkt_dir dir,
                                       uint16_t curSectId,
                                       struct xran_prb_elm *pPrbElm,
...
static inline void populate_ext9_info(int next, uint32_t tti, struct xran_device_ctx *pDevCtx, struct xran_sectionext9_info *ext9)
{
...
static inline int enqueue_cp_pkt_to_tx_sym_ring(uint8_t ruPortId,
                                                int ccId,
                                                uint16_t vfId,
                                                uint32_t tti,
                                                uint8_t mu,
...
#endif
  ring = (struct rte_ring *)pDevCtx->perMu[mu]
             .sFrontHaulTxBbuIoBufCtrl[tti_to_send][ccToSend][antToSend]
             .sBufferList.pBuffers[symToSendOn]
             .pRing;
...
int32_t xran_transmit_cpmsg_prach(void *pHandle,
...
static inline void get_start_sym_and_num_sym(uint8_t xranPortId,
                                             int ccId,
                                             int tti,
                                             uint8_t mu,
                                             xran_pkt_dir dir,
                                             ...
                                             int32_t *numSyms)
{
  if (xran_fs_get_slot_type(xranPortId, ccId, tti, XRAN_SLOT_TYPE_FDD, mu) != 1
    ...
    *startSym = xran_check_symbolrange(((dir == XRAN_DIR_DL) ? XRAN_SYMBOL_TYPE_DL : XRAN_SYMBOL_TYPE_UL),
                                       xranPortId,
                                       ccId,
                                       tti,
                                       pPrbElm->nStartSymb,
                         ...
                         uint8_t sym_id /* sym_id is ignored for non-nbiot calls */)
{
  ...
  pxran_lib_ctx->prach_last_symbol[cc_id] =
  ...
  ret =
      xran_prepare_ctrl_pkt(mbuf, params, cc_id, prach_port_id, seq_id, 0, mu, XRAN_GET_OXU_PORT_ID((xran_device_ctx_t *)pHandle));
  if (ret < 0) {
...
xran_status_t xran_check_cp_dl_sym_to_send(uint32_t cp_dl_tti, uint32_t cp_dl_sym, uint16_t T1a_max_cp_dl, uint8_t mu)
{
  ...
  if (xran_lib_ota_sym_idx_mu[mu] + XRAN_USEC_TO_NUM_SYM(interval, T1a_max_cp_dl)
      == (cp_dl_tti_wrap * N_SYM_PER_SLOT + cp_dl_sym)) {
...
static inline xran_status_t xran_calculate_vMu_time_info(uint8_t mu,
                                                         uint8_t vMu,
                                                         int timerTti2proc,
                                                         uint16_t *vMuTti,
                                                         uint16_t *vMuSfId,
...
xran_status_t xran_reset_vMu_section_db(uint8_t elmIdx,
                                        struct xran_device_ctx *pDevCtx,
                                        uint8_t ccId,
                                        uint8_t ruPortId,
                                        uint8_t ctxId,
                                        uint8_t vMu)
{
  ...
  if ((elmIdx == 0 && xran_get_syscfg_bbuoffload() == 0) || xran_get_syscfg_bbuoffload() == 1) {
    retVal = xran_cp_reset_section_info((void *)pDevCtx, XRAN_DIR_DL, ccId, ruPortId, ctxId, vMu);

    if (unlikely(retVal != XRAN_STATUS_SUCCESS)) {
      print_err("xran_cp_reset_section_info failed dir %u ccId %d ruPortId %hhu ctxId %hhu vMu %hhu",
                XRAN_DIR_DL,
                ccId,
                ruPortId,
                ctxId,
...
*/
xran_status_t
xran_vMu_cp_create_and_send_ssb_pkt(uint8_t xranPortId, uint8_t ruPortId, int dir, int tti2Proc, int ccId, uint8_t vMu, uint8_t mu)
{
  ...
  for (ttiIdx = 0; ttiIdx < timeScalingFactor; ++ttiIdx) {
    ...
    if (likely(pDevCtx->vMuInfo.ssbInfo.sFhSsbTxPrbMapBbuIoBufCtrl[ssbBufId][ccId][ant_id].sBufferList.pBuffers)) {
      if (likely(pDevCtx->vMuInfo.ssbInfo.sFhSsbTxPrbMapBbuIoBufCtrl[ssbBufId][ccId][ant_id].sBufferList.pBuffers->pData)) {
        prbMap = (struct xran_prb_map *)pDevCtx->vMuInfo.ssbInfo.sFhSsbTxPrbMapBbuIoBufCtrl[ssbBufId][ccId][ant_id]
    ...
    for (elmIdx = 0; elmIdx < prbMap->nPrbElm; ++elmIdx) {
      ...
      tempPrbElem.nStartSymb =
          (ssbPrbElem->nStartSymb / timeScalingFactor) + (ttiIdx * (XRAN_NUM_OF_SYMBOL_PER_SLOT / (1 << (vMu - mu))));
      ...
      cpPktGenParams.hdr.timeOffset =
          cpPktGenParams.hdr.startSymId; // 7.5.2.12 - timeOffset points to the same timing pointed by startSymbolId
      ...
      retVal = xran_prepare_ctrl_pkt(ssb_cp_mbuf,
                                     &cpPktGenParams,
                                     ccId,
                                     ruPortId,
                                     seqId,
                                     sectgen_info[elmIdx].info->id,
                                     vMu,
                                     xranPortId);
      ...
      // if(ssbPrbElem->generateCpPkt)
      {
        ...
        if (xran_get_syscfg_bbuoffload()) {
          cp_sent =
              enqueue_cp_pkt_to_tx_sym_ring(ruPortId, ccId, vfId, tti2Proc, mu, pDevCtx, ssb_cp_mbuf, XRAN_DIR_DL, timerMuStartSym);
        } else {
...
int32_t xran_cp_create_and_send_section(uint8_t xranPortId,
                                        uint8_t ruPortId,
                                        int dir,
                                        int tti,
                                        int ccId,
                                        ...
                                        uint8_t bbuoffload)
{
  ...
  bool isNb375 =
      ((XRAN_DIR_UL == dir) && (mu == XRAN_NBIOT_MU) && (pDevCtx->fh_cfg.perMu[mu].nbIotUlScs == XRAN_NBIOT_UL_SCS_3_75) ? true
                                                                                                                         : false);
  ...
  if (0 == pDevCtx->RunSlotPrbMapBySymbolEnable) {
    ...
    for (; curPrbElm < numPrbElm; curPrbElm++) {
      ...
      if ((startSym < 0 || numSyms <= 0)) {
        ...
        print_dbg("Skip section %d due to invalid symbol range - TTI = %d [%d:%d], [%d:%d]",
                  curPrbElm,
                  tti,
                  pPrbElm->nStartSymb,
                  pPrbElm->numSymb,
      ...
      if (useSectType3) {
        ...
        cpPktGenParams.hdr.fftSize =
            xran_get_conf_fftsize((void *)pDevCtx, mu); /*Will be selected based on which numerology is getting processed*/
      }
      ...
      if ((category == XRAN_CATEGORY_B) && (pPrbElm->bf_weight_update)) {
        if (pPrbElm->bf_weight.extType == 1) /* Prepare section data for ext-1 */
        {
          ...
          curExtInSect++;
        } else if (pPrbElm->bf_weight.extType == 11) { /* This should get called only once for ext11 */
          info->ef = 1;
          ...
          if (pPrbElm->bf_weight.extType == 1) /* Using Extension 1 */
          {
            ...
            if (pPrbElm->bf_weight.p_ext_start) {
              ...
              mbuf = xran_attach_cp_ext_buf(pPrbElm->bf_weight.p_ext_start,
                                            ...
                                            pPrbElm->bf_weight.ext_section_sz,
                                            p_share_data);
            } else {
            ...
            if (xran_cp_attach_ext_buf(mbuf,
                                       (uint8_t *)pPrbElm->bf_weight.p_ext_start,
                                       pPrbElm->bf_weight.maxExtBufSize,
                                       shared_info)
                < 0) {
              rte_pktmbuf_free(mbuf);
              ret = -1;
    ...
    for (curPrbElm = 0, j = 0; j < numPrbElm; j++) {
      ...
      sect_geninfo[curPrbElm].info->reMask =
          (pPrbElm->reMask ? pPrbElm->reMask : 0xfff); /*Taking reMask from prbElm, setting to 0xfff if 0*/
