...
struct xran_timing_source_ctx xran_timerCtx = {.state = XRAN_TMTHREAD_STAT_STOP, .sleeptime.tv_nsec = 1E3};
...
long xran_timingsource_poll_next_tick(long interval_ns, unsigned long *used_tick)
{
  ...
  while (1) {
    ...
    if ((delta > 0) || (delta < 0 && labs(delta) < THRESHOLD)) {
      ...
      if (debugStop && (debugStopCount > 0) && (xran_dev_get_ctx_by_id(0)->fh_counters.tx_counter >= debugStopCount)) {
        uint64_t t1;
      ...
      if (pTmCtx->current_second != p_cur_time->tv_sec) {
        ...
        if (debugStop) {
          if (p_cur_time->tv_sec > pTmCtx->started_time.tv_sec && ((p_cur_time->tv_sec % SEC_MOD_STOP) == 0)) {
            ...
            uint32_t frame_id =
                XranGetFrameNum(tti, xran_getSfnSecStart(), SUBFRAMES_PER_SYSTEMFRAME, SLOTNUM_PER_SUBFRAME(interval_us));
        ...
        xran_lib_ota_sym_idx_mu[timerMu] =
            XranIncrementSymIdx(xran_lib_ota_sym_idx_mu[timerMu], XRAN_NUM_OF_SYMBOL_PER_SLOT * slots_per_subframe[timerMu]);
      ...
      if (debugStop && delta < interval_ns * 10)
        MLogTask(PID_TIME_SYSTIME_POLL,
                 (p_last_time->tv_sec * NSEC_PER_SEC + p_last_time->tv_nsec),
                 (p_cur_time->tv_sec * NSEC_PER_SEC + p_cur_time->tv_nsec));
#else
      ...
      if (likely((xran_if_current_state == XRAN_RUNNING) || (xran_if_current_state == XRAN_OWDM))) {
        ...
        if ((p_cur_time->tv_sec * NSEC_PER_SEC + p_cur_time->tv_nsec - sym_start_time)
            >= (interval_ns / XRAN_INTRA_SYM_MAX_DIV) * (xran_intra_sym_div[timerMu] + 1)) {
...
long xran_timingsource_sleep_next_tick(long interval)
{
  ...
  target_time =
      (start_time.tv_sec * NSEC_PER_SEC + start_time.tv_nsec + interval * NSEC_PER_USEC) / (interval * NSEC_PER_USEC) * interval;
