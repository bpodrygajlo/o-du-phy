...
const char *xran_pcid_str_type(struct xran_device_ctx *p_dev, int ant)
{
  ...
  if (ant < xran_get_num_eAxcUl(p_dev))
    ...
    return "PRACH";
  else if (ant >= (xran_get_num_eAxcUl(p_dev) + num_prach)
           && ant < (xran_get_num_eAxcUl(p_dev) + num_prach) + xran_get_num_ant_elm(p_dev))
    return " SRS ";
  else
    return " N/A ";
}
...
/* eCPRI flow supported with DPDK 21.02 or later */
  ...
  for (cc = 0; cc < num_cc; cc++) {
    for (ant = 0; ant < num_eAxc; ant++) {
      ...
      printf("%s: p %d vf %d qi %d 0x%p UP: dir %d cc %d (%d) ant %d (%d) type %s",
             __FUNCTION__,
             xran_port_id,
             vf_id,
             rx_q[vf_id],
             &eth_ctx->vf_and_q2cid[vf_id][rx_q[vf_id]],
             dir,
             cc,
             eth_ctx->vf_and_q2cid[vf_id][rx_q[vf_id]].ccId,
             ant,
             eth_ctx->vf_and_q2cid[vf_id][rx_q[vf_id]].ruPortId,
             xran_pcid_str_type(p_dev, ant));
...
int32_t xran_init_vfs_mapping(void *pHandle)
{
  ...
  if (eth_ctx->io_cfg.nEthLinePerPort != (total_vf_cnt >> (1 - eth_ctx->io_cfg.one_vf_cu_plane))) {
    print_err("Invalid total_vf_cnt - %d [expected %d]",
              total_vf_cnt,
