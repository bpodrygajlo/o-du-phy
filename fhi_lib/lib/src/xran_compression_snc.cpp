...
int32_t xranlib_decompress_avxsnc(const struct xranlib_decompress_request *request, struct xranlib_decompress_response *response)
{
  ...
  //         block_idx_bytes  += ((3 * compressedDataInput.iqWidth) + 1) *
  //         std::min((int16_t)BlockFloatCompander::k_maxNumBlocks,(int16_t)16); remRBs -= 16;
  //     }else if(remRBs >= 4){
  //         compressedDataInput.numBlocks = 4;
  //         decom_fn(compressedDataInput, &expandedDataOut);
  //         len  += 4*compressedDataInput.numDataElements;
  //         block_idx_bytes  += ((3 * compressedDataInput.iqWidth) + 1) *
  //         std::min((int16_t)BlockFloatCompander::k_maxNumBlocks,(int16_t)4); remRBs -=4;
  //     }else if (remRBs >= 1){
  //         compressedDataInput.numBlocks = 1;
  //         decom_fn(compressedDataInput, &expandedDataOut);
  //         len  += 1*compressedDataInput.numDataElements;
  //         block_idx_bytes  += ((3 * compressedDataInput.iqWidth) + 1) *
  //         std::min((int16_t)BlockFloatCompander::k_maxNumBlocks,(int16_t)1); remRBs = remRBs - 1;
  //     }
  // }
...
int32_t xranlib_compress_avxsnc_bfw(const struct xranlib_compress_request *request, struct xranlib_compress_response *response)
{
  ...
  response->len = (((expandedDataInput.numDataElements * expandedDataInput.iqWidth) >> 3) + 1) * request->numRBs;
