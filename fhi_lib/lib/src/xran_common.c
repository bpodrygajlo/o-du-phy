...
extern int32_t xran_validate_seq_id(void *arg, uint8_t CC_ID, uint8_t Ant_ID, uint8_t slot_id, union ecpri_seq_id *seq_id);

/* Today, we are using specific sectionId ranges for different numerologies.
 * sectionId allocation is totally upto DU and this approach avoids maintaining mapping and cycles to retrieve per section per
 * packet In case there is some restriction on sectionId usage, Following code should be enabled to maintain and derive the
 * sectionId <--> mu mapping
 */
...
static inline int xran_rx_timing_window_check(struct xran_device_ctx *p_dev_ctx,
                                              int tti,
                                              uint8_t symId,
                                              uint8_t mu,
                                              uint32_t pktFrameId,
...
#define MAX_SYM_IDX_FROM_PACKET(interval) (256 * SLOTS_PER_SYSTEMFRAME(interval) * XRAN_NUM_OF_SYMBOL_PER_SLOT)
  ...
  if (xran_get_syscfg_appmode() == O_DU) {
    if ((0 == otaTti) && (tti != 0)) { // When received packet is of last tti of a sec
    ...
    if (symIdxDeadlineMax < otaSymIdx) {
      print_dbg(
          "symUpUlUb=%d, {pktFId=%u, otaFId=%u}, {pktSfId=%u, otaSfId=%u}, \t {pktSlId=%u, otaSlId=%u}, {pktSym=%u, otaSym=%u}, "
          "{pktTti=%u, otaTti=%u},otaSymId = %d, xran_lib_ota_sym_idx_mu[mu] = %d\n",
          p_dev_ctx->perMu[mu].sym_up_ul_ub,
          pktFrameId,
          otaFrameId,
          pktSfId,
          otaSfId,
          ...
          otaSlotId,
          symId,
          xran_lib_ota_sym_idx_mu[mu] % XRAN_NUM_OF_SYMBOL_PER_SLOT,
          tti,
          otaTti,
          otaSymId,
      ...
      print_dbg(
          "symUpUlLb=%d, {pktFId=%u, otaFId=%u}, {pktSfId=%u, otaSfId=%u}, \t {pktSlId=%u, otaSlId=%u}, {pktSym=%u, otaSym=%u}, "
          "{pktTti=%u, otaTti=%u},otaSymId = %d, xran_lib_ota_sym_idx_mu[mu] = %d\n",
          p_dev_ctx->perMu[mu].sym_up_ul_lb,
          pktFrameId,
          otaFrameId,
          pktSfId,
          otaSfId,
    ...
    if ((0 == tti) && (otaTti != 0)) { // When received packet is of first tti of a sec with otaTti being  previous tti
...
int process_mbuf_batch(struct rte_mbuf *pkt_q[], void *handle, int16_t num, struct xran_eaxc_info *p_cid, uint32_t *ret_data)
{
  ...
  if (countCompHdrBytes) {
    ...
    for (i = 0; i < MBUFS_CNT; ++i) {
      ...
      if (unlikely(ecpri_hdr[i] == NULL || radio_hdr[i] == NULL || data_hdr[i] == NULL || data_compr_hdr[i] == NULL
                   || iq_samp_buf[i] == NULL)) {
    ...
    for (i = 0; i < MBUFS_CNT; ++i) {
      ...
      if (unlikely(ecpri_hdr[i] == NULL || radio_hdr[i] == NULL || data_hdr[i] == NULL || iq_samp_buf[i] == NULL)) {
        num_bytes[i] = 0; /* packet too short */
      }
  ...
  for (i = 0; i < MBUFS_CNT; ++i) {
    if (num_bytes[i] > 0) {
      ...
      tti = frame_id[i] * SLOTS_PER_SYSTEMFRAME(interval_us_local) + subframe_id[i] * SLOTNUM_PER_SUBFRAME(interval_us_local)
            + slot_id[i];
      ...
      if (xran_get_syscfg_appmode() == O_DU && p_dev_ctx->fh_cfg.srsEnable && Ant_ID[i] >= p_dev_ctx->srs_cfg.srsEaxcOffset
        ...
        ++p_dev_ctx->fh_counters.Rx_on_time;
                     ...
                     -1 == xran_rx_timing_window_check(p_dev_ctx, tti, symb_id[i], mu[i], frame_id[i], subframe_id[i], slot_id[i]))
      ...
      if (xran_get_syscfg_appmode() == O_RU && Ant_ID[i] >= p_dev_ctx->csirs_cfg.csirsEaxcOffset
          && (Ant_ID[i] < (p_dev_ctx->csirs_cfg.csirsEaxcOffset + XRAN_MAX_CSIRS_PORTS)) && p_dev_ctx->fh_cfg.csirsEnable) {
      ...
      } else if (Ant_ID[i] >= p_dev_ctx->srs_cfg.srsEaxcOffset
                 && (Ant_ID[i] < (p_dev_ctx->srs_cfg.srsEaxcOffset + xran_get_num_ant_elm(p_dev_ctx)))
  ...
  for (j = 0; j < num_srs; ++j) {
    ...
    print_dbg("SRS receiving symbol %d, size=%d bytes\n",
              symb_id[i],
              symbol_total_bytes_batch[p_dev_ctx->xran_port_id][CC_ID[i]][Ant_ID[i]][seq_id[i]]);
  ...
  for (j = 0; j < num_pusch; ++j) {
    ...
    pRbMap = (struct xran_prb_map *)p_dev_ctx->perMu[mu[i]]
                 .sFrontHaulRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][CC_ID[i]][Ant_ID[i]]
    ...
    if (likely(pRbMap) && likely(p_dev_ctx->fh_cfg.ru_conf.byteOrder == XRAN_NE_BE_BYTE_ORDER)) {
      ...
      do {
        ...
        if (unlikely(npkts >= XRAN_MAX_RX_PKT_PER_SYM)) {
          if (firstprint == 0)
            print_err("batch:(%d : %d : %d : %d)Received %d type-1 packets on symbol %u\n",
                      frame_id[i],
                      subframe_id[i],
                      subframe_id[i],
                      Ant_ID[i],
...
int32_t process_mbuf(struct rte_mbuf *pkt, void *handle, struct xran_eaxc_info *p_cid)
{
  ...
  num_bytes = xran_extract_iq_samples(pkt,
                                      ...
                                      &compMeth,
                                      &iqWidth,
                                      XRAN_GET_OXU_PORT_ID(p_dev_ctx));
  ...
  int tti = frame_id * SLOTS_PER_SYSTEMFRAME(interval_us_local) + subframe_id * SLOTNUM_PER_SUBFRAME(interval_us_local) + slot_id;
  ...
  if (xran_get_syscfg_appmode() == O_RU && Ant_ID >= p_dev_ctx->csirs_cfg.csirsEaxcOffset
      && (Ant_ID < (p_dev_ctx->csirs_cfg.csirsEaxcOffset + XRAN_MAX_CSIRS_PORTS)) && p_dev_ctx->fh_cfg.csirsEnable) {
    ...
    if (seq.bits.e_bit == 1) {
      print_dbg("CSI-RS receiving symbol %d, size=%d bytes\n",
                symb_id,
                symbol_total_bytes[p_dev_ctx->xran_port_id][CC_ID][Ant_ID][seq.bits.seq_id]);
  ...
  /* do not validate for NDM SRS */
           ...
           && (Ant_ID < p_dev_ctx->srs_cfg.srsEaxcOffset + xran_get_num_ant_elm(p_dev_ctx)) && p_dev_ctx->fh_cfg.srsEnable) {
    if (unlikely(xran_validate_numprbs(mu, start_prbu, num_prbu, p_dev_ctx) != XRAN_STATUS_SUCCESS))
      return MBUF_FREE;
    ...
    if (Ant_ID >= PrachCfg->prachEaxcOffset && Ant_ID < (PrachCfg->prachEaxcOffset + xran_get_num_eAxc(p_dev_ctx))
      ...
      if (seq.bits.e_bit == 1) {
        ...
        if (symbol_total_bytes[p_dev_ctx->xran_port_id][CC_ID][Ant_ID][seq.bits.seq_id]) {
          ...
          do {
            ...
            if (compMeth != XRAN_COMPMETHOD_MODULATION && mb_free == MBUF_KEEP && nSectionLen < num_bytes) {
              // check have other sections
              // move to next section
              rte_pktmbuf_adj(pkt, nSectionLen);

              // get section data header
              num_bytes = xran_extract_iq_samples_dataheader(pkt,
                                                             &iq_samp_buf,
                                                             &num_prbu,
                                                             &start_prbu,
                                                             &sym_inc,
                                 ...
                                 uint8_t oxu_port_id)
{
  ...
  for (idx = 0; idx < num_sections; idx++) {
    ...
    // idx*nprb_per_section),prb_num,num_sections,nprb_per_section,curr_sect_id);
                                ...
                                uint8_t mu)
{
  ...
  if (p_dev_ctx != NULL) {
    ...
    hdr_len = sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr) + sizeof(struct data_section_hdr);
    if ((compMeth != XRAN_COMPMETHOD_NONE) && (staticEn == XRAN_COMP_HDR_TYPE_DYNAMIC))
    ...
    for (loop = 0; loop < 3; loop++) {
      ...
      if (sent) {
        ...
        // p_dev_ctx->send_upmbuf2ring(send_mb, ETHER_TYPE_ECPRI, xran_map_ecpriPcid_to_vf(p_dev_ctx, direction, CC_ID,
        // RU_Port_ID));
      }
    }
...
bool xran_check_if_late_transmission(uint32_t ttiForRing, uint32_t symIdForRing, uint8_t mu)
{
  ...
  if (ttiDiff > 0) {
    if (ttiDiff > 2) { /* ota ahead and rolled over */
    ...
    ttiDiff *= (-1);
    if (ttiDiff > 2) { /* tti has rolled over */
      return false;
    } else {
      print_err("tti Too late: otaTti=%u forRingTti=%u\n", otaTti, ttiForRing);
      return true;
