                                           ...
                                           enum xran_comp_hdr_type staticEn)
{
  ...
  uint16_t ecpri_payl_size = payl_size + sizeof(struct radio_app_common_hdr) + XRAN_ECPRI_HDR_SZ; // xran_get_ecpri_hdr_size();
 ...
 * @return void
 */
static inline void build_application_layer(struct rte_mbuf *mbuf, const struct radio_app_common_hdr *app_hdr_input)
{
  char *pChar = rte_pktmbuf_mtod(mbuf, char *);
  struct radio_app_common_hdr *app_hdr =
      (struct radio_app_common_hdr *)(pChar + sizeof(struct rte_ether_hdr) + sizeof(struct xran_ecpri_hdr));
 ...
 * @return void
 */
static inline void build_section_hdr(struct rte_mbuf *mbuf, const struct data_section_hdr *sec_hdr, uint32_t offset)
{
  char *pChar = rte_pktmbuf_mtod(mbuf, char *);
 ...
 * @return void
 */
static inline void build_compression_hdr(struct rte_mbuf *mbuf,
                                         const struct data_section_compression_hdr *compr_hdr,
                                         uint32_t offset)
{
  char *pChar = rte_pktmbuf_mtod(mbuf, char *);
  struct data_section_compression_hdr *compression_hdr = (struct data_section_compression_hdr *)(pChar + offset);
                                ...
                                uint8_t oxu_port_id)
{
  ...
  struct radio_app_common_hdr *radio_hdr = (void *)rte_pktmbuf_adj(mbuf, sizeof(*ecpri_hdr));
  if (radio_hdr == NULL)
  ...
  struct data_section_hdr *data_hdr = (void *)rte_pktmbuf_adj(mbuf, sizeof(*radio_hdr));
  if (data_hdr == NULL)
    return 0; /* packet too short */
  ...
  *sect_id = data_hdr->fields.sect_id;
  ...
  if (expect_comp) {
    ...
    if (staticComp != XRAN_COMP_HDR_TYPE_STATIC) {
      data_compr_hdr = (void *)rte_pktmbuf_adj(mbuf, sizeof(*data_hdr));
      ...
      *iqWidth = data_compr_hdr->ud_comp_hdr.ud_iq_width;
      const uint8_t *compr_param = (void *)rte_pktmbuf_adj(mbuf, sizeof(*data_compr_hdr));
                                           ...
                                           uint8_t *iqWidth)
{
  ...
  /* Process data section hdr */
  struct data_section_hdr *data_hdr = (struct data_section_hdr *)((char *)mbuf->buf_addr + mbuf->data_off);
                                       ...
                                       uint8_t oxu_port_id)
{
  ...
  offset = sizeof(struct rte_ether_hdr) + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr);
  for (idx = 0; idx < num_sections; idx++) {
    build_section_hdr(mbuf, &(params[idx].sec_hdr), offset);

    offset += sizeof(struct data_section_hdr);
    if ((params[idx].compr_hdr_param.ud_comp_hdr.ud_comp_meth != XRAN_COMPMETHOD_NONE)
    ...
    // ret_val = (do_copy ? append_iq_samples_ex(mbuf, offset, iq_data_start, iq_data_num_bytes/num_sections, iq_buf_byte_order,
    // do_copy) : iq_data_num_bytes/num_sections);
...
int32_t xran_prepare_iq_symbol_portion_appand(struct rte_mbuf *mbuf,
                                              const void *iq_data_start,
                                              const enum xran_input_byte_order iq_buf_byte_order,
                                              const uint32_t iq_data_num_bytes,
                                              struct xran_up_pkt_gen_params *params,
