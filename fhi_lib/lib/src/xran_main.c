...
uint16_t xran_max_frame = 1023; /**< value of max frame used. expected to be 99 (old compatibility mode) and 1023 as per
                                   section 9.7.2    System Frame Number Calculation */
...
int32_t xran_init_srs(struct xran_fh_config *pConf, struct xran_device_ctx *pDevCtx)
{
...
int32_t xran_init_prach_lte(struct xran_fh_config *pConf, struct xran_device_ctx *pDevCtx)
{
  ...
  offset = pPRACHConfig->nPrachFreqOffset - pConf->perMu[0].nULRBs * N_SC_PER_PRB * 12
  ...
  for (i = 0; i < XRAN_MAX_SECTOR_NR; i++) {
    ...
    pDevCtx->prach_last_symbol[i] =
        pPrachCPConfig->startSymId + pPrachCPConfig->numSymbol * pPrachCPConfig->occassionsInPrachSlot - 1;
  }
  ...
  if (pConf->log_level) {
    printf("xRAN open PRACH config(LTE): ConfIdx %u, preambleFmrt %u, systemframe %u\n",
           nPrachConfIdx,
           preambleFormat,
           pPrachCfgTbl->frameNum);
    printf("PRACH: x %u, y[0] %u, y[1] %u ... [ ", pPrachCPConfig->x, pPrachCPConfig->y[0], pPrachCPConfig->y[1]);
    for (i = 0; i < SUBFRAMES_PER_SYSTEMFRAME; i++)
...
int32_t xran_init_prach(struct xran_fh_config *pConf, struct xran_device_ctx *pDevCtx, enum xran_ran_tech xran_tech, uint8_t mu)
{
  int32_t i;
  uint8_t slotNr;
  ...
  if (XRAN_NBIOT_MU == mu) {
    ...
    if (pConf->log_level)
      printf(
          "xRAN open PRACH config: Numerology %u ConfIdx %u, preambleFmrt %u startsymb %u, numSymbol %u, occassionsInPrachSlot "
          "%u\n",
          nNumerology,
          nPrachConfIdx,
          preambleFmrt,
          pxRANPrachConfigTable->startingSym,
          pxRANPrachConfigTable->duration,
          pxRANPrachConfigTable->occassionsInPrachSlot);
    ...
    pPrachCPConfig->numPrbc =
        (preambleFmrt >= FORMAT_A1)
            ? 12
            : 72; /*TODO: This should change for NB-IOT, only 1 PRB is to be allocated - check format accordingly*/
    pPrachCPConfig->timeOffset = pxranPreambleforLRA->nRaCp;
    pPrachCPConfig->freqOffset =
        xran_get_prach_freqoffset(pConf, mu, pPRACHConfig->nPrachSubcSpacing, pPRACHConfig->nPrachFreqStart);
    ...
    if (pConf->log_level)
      printf("PRACH: x %u y[0] %u, y[1] %u prach slot: %u ..",
             pPrachCPConfig->x,
             pPrachCPConfig->y[0],
             pPrachCPConfig->y[1],
             pxRANPrachConfigTable->slotNr[0]);
...
uint32_t xran_slotid_convert(
    uint16_t slotId,
    uint16_t dir) // dir = 0, from PHY slotid to xran spec slotid as defined in 5.3.2, dir=1, from xran slotid to phy slotid
{
  return slotId;
#if 0
...
void xran_process_nbiot_prach_cp(struct xran_device_ctx *pDevCtx, uint8_t mu)
{
  ...
  uint16_t numSymsForT1a_max_cp_dl =
      (pDevCtx->fh_cfg.perMu[mu].T1a_max_cp_dl * 1000) / (interval_us_local * 1000 / XRAN_NUM_OF_SYMBOL_PER_SLOT);
  ...
  uint16_t frameId =
      XranGetFrameNum(tti, xran_getSfnSecStart(), SUBFRAMES_PER_SYSTEMFRAME, SLOTNUM_PER_SUBFRAME(interval_us_local));
  ...
  if ((frameId % (pPrachCPConfig->periodicity / SUBFRAMES_PER_SYSTEMFRAME) == 0)) {
    ...
    for (uint8_t i = 0; i < NBIOT_PRACH_NUM_SYM_GROUPS; i++) {
      if (pDevCtx->perMu[mu].sfSymToXmitPrach[i].sf == sfId && pDevCtx->perMu[mu].sfSymToXmitPrach[i].sym == sym_id) {
        for (uint16_t antId = 0; antId < num_eAxc; antId++) {
          ...
          for (uint16_t ccId = 0; ccId < num_CCPorts; ccId++) {
            ...
            ret = generate_cpmsg_prach(pDevCtx,
                                       &params,
                                       sect_geninfo,
                                       mbuf,
                                       pDevCtx,
            ...
            if (ret == XRAN_STATUS_SUCCESS) {
              if (unlikely(xran_transmit_cpmsg_prach(pDevCtx, &params, sect_geninfo, mbuf, pDevCtx, tti, ccId, portId, seqid, mu)
                           != XRAN_STATUS_SUCCESS)) {
               ...
               */
              xran_cp_reset_section_info(pDevCtx,
                                         XRAN_DIR_UL,
                                         ccId,
                                         portId,
                                         (0 + sfId * SLOTNUM_PER_SUBFRAME(xran_fs_get_tti_interval(mu))) % XRAN_MAX_SECTIONDB_CTX,
                                         mu);
            }
          } /*ccId*/
        }
...
void sym_ota_cb(void *arg, unsigned long *used_tick, uint8_t mu)
{
  ...
  if (XRAN_NBIOT_MU == mu && pDevCtx->perMu[mu].enablePrach && xran_get_syscfg_appmode() == O_DU) {
...
void sym_ota_cb_ebbu_offload(void *arg, unsigned long *used_tick, uint8_t mu)
{
  ...
  if (XRAN_NBIOT_MU == mu && p_xran_dev_ctx->perMu[mu].enablePrach && xran_get_syscfg_appmode() == O_DU) {
...
uint32_t xran_schedule_to_worker(enum xran_job_type_id job_type_id, struct xran_device_ctx *pDevCtx)
{
...
void tti_ota_cb(struct rte_timer *tim, uint8_t mu)
{
...

  {
    ...
    reg_sfn = XranGetFrameNum(reg_tti, xran_getSfnSecStart(), SUBFRAMES_PER_SYSTEMFRAME, SLOTNUM_PER_SUBFRAME(interval_us)) * 10
              + XranGetSubFrameNum(reg_tti, SLOTNUM_PER_SUBFRAME(interval_us), SUBFRAMES_PER_SYSTEMFRAME);
    /* subframe and slot */
    ...
    frameId = XranGetFrameNum(xran_lib_ota_tti_base,
                              xran_getSfnSecStart(),
                              SUBFRAMES_PER_SYSTEMFRAME,
                              SLOTNUM_PER_SUBFRAME(interval_us_local));

    xran_get_slot_idx(0, &nFrameIdx, &nSubframeIdx, &nSlotIdx, &nSecond, timerMu);
    nSfIdx = nFrameIdx * SUBFRAMES_PER_SYSTEMFRAME * nNrOfSlotInSf + nSubframeIdx * nNrOfSlotInSf + nSlotIdx;
    mlogVarCnt = 0;
...
void tti_ota_cb_ebbu_offload(struct rte_timer *tim, void *arg, uint8_t mu)
{
  ...
  if (xran_timingsource_get_numerology() == mu) { /* will get entered for highest configured mu */
    /* To match TTbox */
  ...
  subframe_id = XranGetSubFrameNum(pCtx->ebbu_offload_ota_tti_cnt_mu[PortId][mu],
                                   SLOTNUM_PER_SUBFRAME(interval_us_local),
                                   SUBFRAMES_PER_SYSTEMFRAME);
  frame_id = XranGetFrameNum(pCtx->ebbu_offload_ota_tti_cnt_mu[PortId][mu],
                             xran_getSfnSecStart(),
                             SUBFRAMES_PER_SYSTEMFRAME,
                             SLOTNUM_PER_SUBFRAME(interval_us_local));
  ...
  if (xran_get_syscfg_appmode() == O_DU)
    ...
    pTCtx[(pCtx->ebbu_offload_ota_tti_cnt_mu[PortId][mu] & 1)].tti_to_process =
        pTCtx[(pCtx->ebbu_offload_ota_tti_cnt_mu[PortId][mu] & 1) ^ 1].tti_to_process;
...
int32_t xran_prepare_cp_dl_slot(uint16_t xran_port_id,
                                uint32_t nSlotIdx,
                                uint32_t nCcStart,
                                uint32_t nCcNum,
                                uint32_t nSymMask,
        ...
        if (xran_fs_get_slot_type(PortId, ccId, tti, XRAN_SLOT_TYPE_DL, mu) == 1) {
          if (pDevCtx->perMu[mu].sFrontHaulTxPrbMapBbuIoBufCtrl[bufId][ccId][antId].sBufferList.pBuffers) {
            if (pDevCtx->perMu[mu].sFrontHaulTxPrbMapBbuIoBufCtrl[bufId][ccId][antId].sBufferList.pBuffers->pData) {
              xran_cp_create_and_send_section(PortId,
                                              ruPortId,
                                              XRAN_DIR_DL,
                                              tti,
                                              ccId,
                                              (struct xran_prb_map *)pDevCtx->perMu[mu]
                                                  .sFrontHaulTxPrbMapBbuIoBufCtrl[bufId][ccId][antId]
          ...
          if (pDevCtx->vMuInfo.ssbInfo.ssbMu > mu) /* If SSB uses a different numerology, process SSB as a virtual numerology */
          {
            if (mu == 3 && pDevCtx->vMuInfo.ssbInfo.ssbMu == 4) {
              ...
              if (unlikely(xran_vMu_cp_create_and_send_ssb_pkt(PortId,
                                                               SsbRuPortId,
                                                               XRAN_DIR_DL,
                                                               tti,
                                                               ccId,
                                                               pDevCtx->vMuInfo.ssbInfo.ssbMu,
                                                               mu)
                           != XRAN_STATUS_SUCCESS)) {
    ...
    if (pDevCtx->csirsEnable && nAntStart == 0) {
      ...
      for (antId = 0; antId < XRAN_MAX_CSIRS_PORTS; antId++) {
        ...
        for (ccId = nCcStart; (ccId < (nCcStart + nCcNum) && ccId < num_CCPorts); ccId++) {
          ...
          if (xran_fs_get_slot_type(PortId, ccId, tti, XRAN_SLOT_TYPE_DL, mu) == 1) {
            pBufList =
                &(pDevCtx->perMu[mu].sFHCsirsTxPrbMapBbuIoBufCtrl[bufId][ccId][antId].sBufferList); /* To shorten reference */
            if (pBufList->pBuffers && pBufList->pBuffers->pData) {
              /*ret = */ xran_cp_create_and_send_section(PortId,
                                                         ruPortId,
                                                         XRAN_DIR_DL,
                                                         tti,
                                                         ccId,
...
void tx_cp_dl_cb(struct rte_timer *tim, void *arg)
{
  ...
  if (first_call && pDevCtx->enableCP) {
    ...
    for (iloop = prevTti; iloop <= (prevTti + 1); ++iloop) {
      ...
      frameId =
          XranGetFrameNum(cp_dl_tti, xran_getSfnSecStart(), SUBFRAMES_PER_SYSTEMFRAME, SLOTNUM_PER_SUBFRAME(interval_us_local));
      if (cp_dl_tti == 0) {
      ...
      for (antId = 0; antId < num_eAxc; ++antId) {
        ...
        for (ccId = 0; ccId < num_CCPorts; ccId++) {
          ...
          if ((xran_lib_ota_sym_idx_mu[mu] % N_SYM_PER_SLOT == 0
              ...
              || (0 == pDevCtx->dlCpProcBurst && pDevCtx->prbElmProcInfo[bufId][ccId][antId].numSymsRemaining == 0
          ...
          if (xran_fs_get_slot_type(PortId, ccId, cp_dl_tti, XRAN_SLOT_TYPE_DL, mu) == 1) {
            ...
            if (pDevCtx->vMuInfo.ssbInfo.ssbMu > mu) /* If SSB uses a different numerology, process SSB as a virtual numerology */
            {
              if (mu == 3 && pDevCtx->vMuInfo.ssbInfo.ssbMu == 4) {
                uint8_t SsbRuPortId = antId + pDevCtx->vMuInfo.ssbInfo.ruPortId_offset;
                retVal = xran_vMu_cp_create_and_send_ssb_pkt(PortId,
                                                             SsbRuPortId,
                                                             XRAN_DIR_DL,
                                                             cp_dl_tti,
                                                             ccId,
      ...
      if (pDevCtx->csirsEnable) {
        ...
        for (antId = 0; antId < XRAN_MAX_CSIRS_PORTS; antId++) {
          ...
          for (ccId = 0; ccId < num_CCPorts; ccId++) {
            ...
            if ((xran_lib_ota_sym_idx_mu[mu] % N_SYM_PER_SLOT == 0
                 && (cp_dl_tti == tti) /*Ensure that we don't reset the DB for previous tti*/)
                || (0 == pDevCtx->dlCpProcBurst && pDevCtx->prbElmProcCSIInfo[bufId][ccId][ruPortId].numSymsRemaining == 0
                    && pDevCtx->DynamicSectionEna == 0)) {
              xran_cp_reset_section_info(pHandle, XRAN_DIR_DL, ccId, ruPortId, ctxId, mu);
            }
            if (xran_fs_get_slot_type(PortId, ccId, cp_dl_tti, XRAN_SLOT_TYPE_DL, mu) == 1) {
...
void rx_ul_static_srs_cb(struct rte_timer *tim, void *arg)
{
  ...
  rx_tti = (rx_tti + xran_fs_get_max_slot(mu) - 1 - pDevCtx->perMu[mu].deadline_slot_advance[XRAN_SLOT_HALF_CB])
           % xran_fs_get_max_slot(mu);
...
void rx_ul_deadline_one_fourths_cb(struct rte_timer *tim, void *arg)
{
  ...
  rx_tti =
      (rx_tti + xran_fs_get_max_slot(mu) - pDevCtx->perMu[mu].deadline_slot_advance[XRAN_SLOT_1_4_CB]) % xran_fs_get_max_slot(mu);
  ...
  if (xran_timingsource_get_numerology() == mu && pTmCtx->ttiCb[XRAN_CB_HALF_SLOT_RX]) {
...
void rx_ul_deadline_half_cb(struct rte_timer *tim, void *arg)
{
  ...
  rx_tti =
      (rx_tti + xran_fs_get_max_slot(mu) - pDevCtx->perMu[mu].deadline_slot_advance[XRAN_SLOT_HALF_CB]) % xran_fs_get_max_slot(mu);
...
void rx_ul_deadline_three_fourths_cb(struct rte_timer *tim, void *arg)
{
  ...
  rx_tti =
      (rx_tti + xran_fs_get_max_slot(mu) - pDevCtx->perMu[mu].deadline_slot_advance[XRAN_SLOT_3_4_CB]) % xran_fs_get_max_slot(mu);
...
void rx_ul_deadline_full_cb(struct rte_timer *tim, void *arg)
{
  ...
  rx_tti = (rx_tti + xran_fs_get_max_slot(mu) - 1 - pDevCtx->perMu[mu].deadline_slot_advance[XRAN_SLOT_FULL_CB])
           % xran_fs_get_max_slot(mu);
  ...
  if (xran_timingsource_get_numerology() == mu && pTmCtx->ttiCb[XRAN_CB_FULL_SLOT_RX]) {
...
void rx_ul_user_sym_cb(struct rte_timer *tim, void *arg)
{
  ...
  if (p_sym_cb_ctx->sym_diff > 0)
    ...
    if (p_timer_ctx->ota_sym_idx >= abs(p_sym_cb_ctx->sym_diff)) {
      ...
      ota_sym_idx = ((xran_timingsource_get_max_ota_sym_idx(mu) + p_timer_ctx->ota_sym_idx) + p_sym_cb_ctx->sym_diff)
                    % xran_timingsource_get_max_ota_sym_idx(mu);
    }
  ...
  if (p_sym_cb_ctx->symCbTimeInfo) {
    ...
    p_sense_time->nFrameIdx = (uint32_t)
        XranGetFrameNum(rx_tti, p_timer_ctx->xran_sfn_at_sec_start, SUBFRAMES_PER_SYSTEMFRAME, SLOTNUM_PER_SUBFRAME(interval));
...
int32_t xran_prepare_cp_ul_slot(uint16_t xran_port_id,
                                uint32_t nSlotIdx,
                                uint32_t nCcStart,
                                uint32_t nCcNum,
                                uint32_t nSymMask,
...
#pragma vector always
    ...
    for (antId = nAntStart; (antId < (nAntStart + nAntNum) && antId < num_eAxc); ++antId) {
      ...
      for (ccId = nCcStart; (ccId < (nCcStart + nCcNum) && ccId < num_CCPorts); ccId++) {
        ...
        if (xran_fs_get_slot_type(PortId, ccId, tti, XRAN_SLOT_TYPE_UL, mu) == 1) {
          pBufList =
              &(pDevCtx->perMu[mu].sFrontHaulRxPrbMapBbuIoBufCtrl[bufId][ccId][antId].sBufferList); /* To shorten reference */
          if (pBufList->pBuffers && pBufList->pBuffers->pData) {
            ret = xran_cp_create_and_send_section(xran_port_id,
                                                  ruPortId,
                                                  XRAN_DIR_UL,
                                                  tti,
                                                  ccId,
    ...
    if (pDevCtx->perMu[mu].enablePrach && mu != XRAN_NBIOT_MU) {
      ...
      if (((frameId % pPrachCPConfig->x) == pPrachCPConfig->y[0]) && (is_prach_slot == 1)) {
        for (antId = nAntStart; (antId < nAntStart + nAntNum) && antId < num_eAxc; antId++) {
          ...
          for (ccId = nCcStart; (ccId < (nCcStart + nCcNum) && ccId < num_CCPorts); ccId++) {
            ...
            for (occasionid = 0; occasionid < occasionloop; occasionid++) {
              ...
              ret = generate_cpmsg_prach(pHandle,
                                         &params,
                                         sect_geninfo,
                                         mbuf,
                                         pDevCtx,
                                         ...
                                         0);
              if (ret == XRAN_STATUS_SUCCESS) {
                if (unlikely(xran_transmit_cpmsg_prach(pHandle, &params, sect_geninfo, mbuf, pDevCtx, tti, ccId, portId, seqid, mu)
                             != XRAN_STATUS_SUCCESS)) {
                  print_err("Error: PRACH CP transmit failed! ru_portId %d ccId %d tti %d\n", portId, ccId, tti);
                }
              } else
    ...
    if (pDevCtx->enableSrsCp) {
      ...
      uint16_t stream2srs =
          xran_get_num_ant_elm(pHandle) / num_eAxc; /* do poportional part of SRS (e.g 64/16=4 per nAntNum equal to 1*/
      for (antId = nAntStart * stream2srs; (antId < ((nAntStart + nAntNum) * stream2srs) && antId < xran_get_num_ant_elm(pHandle)
                                            && antId < XRAN_MAX_ANT_ARRAY_ELM_NR);
           ++antId) {
...
void tx_cp_ul_cb(struct rte_timer *tim, void *arg)
{
  ...
  if (first_call && pDevCtx->enableCP) {
    ...
    if (mu == XRAN_NBIOT_MU && pDevCtx->fh_cfg.perMu[mu].nbIotUlScs == XRAN_NBIOT_UL_SCS_3_75) {
      if (tti % 2) { // tti_to_process is even --> OTA tti is odd
        if (pDevCtx->perMu[mu].ulCpSlotOffset)
          return;
        // else send ul cp packet
      } else { // tti_to_process is odd --> OTA tti is even
...
int32_t xran_handle_rx_pkts(struct rte_mbuf *pkt_q[], uint16_t xport_id, struct xran_eaxc_info *p_cid, uint16_t num, uint16_t vf_id)
{
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (eth_hdr->ether_type == rte_cpu_to_be_16(ETHER_TYPE_ECPRI)) {
      ...
      struct radio_app_common_hdr *radio_hdr = rte_pktmbuf_mtod_offset(pkt, struct radio_app_common_hdr *, sizeof(*ecpri_hdr));
#if 0 /* Cov */
            if (radio_hdr == NULL)
            {
...
#endif
      if (unlikely(xran_get_syscfg_appmode() == O_DU && radio_hdr->data_feature.data_direction == XRAN_DIR_DL)) {
      ...
      if (unlikely(xran_get_syscfg_appmode() == O_RU && ecpri_hdr->cmnhdr.bits.ecpri_mesg_type == 0
      ...
      switch (ecpri_hdr->cmnhdr.bits.ecpri_mesg_type) {
        ...
        case ECPRI_DELAY_MEASUREMENT:
          // ECPRI payload validation
          if (unlikely(expected_ecpri_payload
                       != ((rte_pktmbuf_pkt_len(pkt) - sizeof(union xran_ecpri_cmn_hdr) - 4)))) // Subtracting 4 bytes for FCS
          {
            ++p_dev_ctx->fh_counters.rx_err_ecpri;
            ++p_dev_ctx->fh_counters.rx_err_drop;
            ++p_dev_ctx->fh_counters.rx_counter;
...
int32_t xran_init(int argc, char *argv[], struct xran_fh_init *p_xran_fh_init, char *appName, void **pXranLayerHandle)
{
  ...
  if (p_xran_fh_init->xran_ports * p_io_cfg->nEthLinePerPort * (2 - 1 * p_io_cfg->one_vf_cu_plane) != p_io_cfg->num_vfs) {
    print_err("Incorrect VFs configurations: For %d O-xUs with %d Ethernet ports expected number of VFs is %d. [provided %d]\n",
              p_xran_fh_init->xran_ports,
              p_io_cfg->nEthLinePerPort,
              p_xran_fh_init->xran_ports * p_io_cfg->nEthLinePerPort * (2 - 1 * p_io_cfg->one_vf_cu_plane),
              p_io_cfg->num_vfs);
  }
  ...
  if (p_io_cfg->id == O_DU)
    xran_ethdi_init_dpdk_ports(p_io_cfg,
                               &lcore_id,
...
int32_t xran_sector_get_instances(uint32_t xran_port,
                                  void *pDevHandle,
                                  ...
                                  xran_cc_handle_t *pSectorInstanceHandles)
{
  ...
  for (i = 0; i < nNumInstances; i++) {
    ...
    // printf("  RU%d CC%d: Sector handle: %p Instance: %p\n", pDev->xran_port_id, i, &pSectorInstanceHandles[i],
    // pSectorInstanceHandles[i]);
  }
                                  ...
                                  uint8_t mu)
{
  ...
  if (pDevCtx->pCallback[0] == NULL) {
    ...
    printf("\n%s: [p %d CC  %d] Cb %p cb %p\n",
           __FUNCTION__,
           pDevCtx->xran_port_id,
           0,
           pDevCtx->pCallback[0],
           pDevCtx->pCallbackTag[0]);
  }
                          ...
                          uint8_t mu)
{
  ...
  if (pDevCtx->pPrachCallback[0] == NULL) {
    ...
    printf("\n%s: [p %d CC  %d] Cb %p cb %p\n",
           ...
           pDevCtx->xran_port_id,
           0,
           pDevCtx->pPrachCallback[0],
           pDevCtx->pPrachCallbackTag[0]);
  }
                        ...
                        uint8_t mu)
{
  ...
  if (pDevCtx->pSrsCallback[0] == NULL) {
    ...
    printf("\n%s: [p %d CC  %d] Cb %p cb %p\n",
           ...
           pDevCtx->xran_port_id,
           0,
           pDevCtx->pSrsCallback[0],
           pDevCtx->pSrsCallbackTag[0]);
  }
                             ...
                             uint8_t mu)
{
  ...
  if (pDevCtx->pCsirsCallback[0] == NULL) {
    ...
    printf("\n%s: [p %d CC  %d] Cb %p cb %p\n",
           ...
           pDevCtx->xran_port_id,
           0,
           pDevCtx->pCsirsCallback[0],
           pDevCtx->pCsirsCallbackTag[0]);
  }
...
{
  dst += (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct xran_cp_radioapp_section1_header)
...
{
  dst += (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr)
...
int32_t xran_tx_sym_process_ring(struct rte_ring *r)
{
  assert(r);
...
int32_t xran_dequeue_timing_info_for_tx(void *args)
{
  struct xran_ethdi_ctx *const ctx = xran_ethdi_get_ctx();
  uint16_t xran_port_mask = (uint16_t)((uint64_t)args & 0xFFFF);
  uint16_t current_port;
  ...
  for (current_port = 0; current_port < XRAN_PORTS_NUM; current_port++) {
    if (xran_port_mask & (1 << current_port)) {
      ...
      const uint16_t dequeued =
          rte_ring_dequeue_burst(ctx->up_dl_pkt_gen_ring[current_port], (void **)mbufs, RTE_DIM(mbufs), &remaining);
      if (!dequeued)
        continue;
...
int32_t xran_pkt_gen_process_via_ring(struct rte_ring *r)
{
  assert(r);
  struct rte_mbuf *mbufs[16];
  int i;
...
int32_t xran_tx_symb_ring_processing_func(void *args)
{
  struct xran_ethdi_ctx *const ctx = xran_ethdi_get_ctx();
  uint16_t xran_port_mask = (uint16_t)((uint64_t)args & 0xFFFF);
  uint16_t current_port;
    ...
    for (port_id = starPort_id; port_id < endPort_id; ++port_id) {
      ...
      if (p_dev_ctx) {
        for (mu_idx = 0; mu_idx < p_dev_ctx->fh_cfg.numMUs; mu_idx++) {
          ...
          xran_process_tx_sym_cp_on_ring_opt(p_dev_ctx,
                                             0,
                                             p_dev_ctx->fh_cfg.nCC,
                                             0,
                                             p_dev_ctx->fh_cfg.neAxc,
                                             XRAN_DIR_DL,
                                             mu,
                                             tti,
                                             sym_id);
...
int32_t xran_dl_pkt_via_ring_processing_func(void *args)
{
  struct xran_ethdi_ctx *const ctx = xran_ethdi_get_ctx();
  uint16_t xran_port_mask = (uint16_t)((uint64_t)args & 0xFFFF);
  uint16_t current_port;
...
int32_t xran_processing_timer_only_func(void *args)
{
  rte_timer_manage();
  if (XRAN_STOPPED == xran_if_current_state)
    return -1;
...
int32_t xran_all_tasks(void *arg)
{
...
int32_t xran_eth_trx_tasks(void *arg)
{
  process_dpdk_io(arg);
  return 0;
}
...
/** Function to pefromrm TX and RX on ETH device */
int32_t xran_eth_trx_tasks_ports(void *arg)
{
  // process_dpdk_io(arg);
  rte_timer_manage();
...
int32_t xran_eth_rx_tasks(void *arg)
{
...
int32_t ring_processing_func_per_port(void *args)
{
  struct xran_ethdi_ctx *const ctx = xran_ethdi_get_ctx();
  int32_t i;
...
#define XRAN_SET_WORKER_INFO(p_worker_info, Worker_Id, Thread_name, Task_func, Task_arg, state) \
  p_worker_info = (struct xran_worker_info_s){.WorkerId = Worker_Id,                            \
...
int32_t xran_spawn_workers(void)
{
  ...
  for (i = 0; i < numRUs; ++i) {
    ...
    if (tmpDevCtx != NULL) {
      printf("    O-RU[%d]: Cat-%c CC[%2d] eAxC[%2d]\n",
             ...
             tmpDevCtx->fh_cfg.nCC,
             tmpDevCtx->fh_cfg.neAxc);
    } else {
    ...
    switch (total_num_cores) {
      ...
      case 4:
        ...
        if (xran_get_syscfg_bbuoffload()) {
          ...
          XRAN_SET_WORKER_INFO(p_worker_info[1], 0, "fh_rx_bbdev", ring_processing_func, NULL, 1);
          /** 1 - CP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[2],
                               1,
                               "fh_cp_gen",
                               xran_dl_pkt_ring_processing_func,
                               (void *)(((1L << 1) | (1L << 2) | (1L << 0)) & xran_port_mask),
                               1);
          /** 2 UP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[3],
                               2,
                               "fh_tx_gen",
                               xran_dl_pkt_ring_processing_func,
                               (void *)((1L << 0) & xran_port_mask),
                               1);
      ...
      case 5:
        ...
        if (xran_get_syscfg_bbuoffload()) {
          ...
          XRAN_SET_WORKER_INFO(p_worker_info[3],
                               ...
                               1);
          /** 3 UP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[4],
                               3,
                               "fh_tx_gen",
                               xran_dl_pkt_ring_processing_func,
                               (void *)((1L << 0) & xran_port_mask),
      ...
      case 6:
        if (xran_get_syscfg_appmode() == O_DU) {
          ...
          XRAN_SET_WORKER_INFO(p_worker_info[4],
                               ...
                               1);
          /** 4 UP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[5],
                               4,
                               "fh_tx_gen",
                               xran_dl_pkt_ring_processing_func,
                               (void *)((1L << 0) & xran_port_mask),
    ...
    switch (total_num_cores) {
      ...
      case 3:
        ...
        else /* csx cpu */
        {
          ...
          /* workers */
          /** 0 **/
          XRAN_SET_WORKER_INFO(p_worker_info[1],
                               0,
                               "fh_rx_bbdev",
                               xran_dl_pkt_ring_processing_func,
                               (void *)((1L << 0 | 1L << 1) & xran_port_mask),
                               1);
          for (i = 1; i < numRUs; i++) {
            arr_job2wrk_id[i][XRAN_JOB_TYPE_CP_DL] = 0;
            arr_job2wrk_id[i][XRAN_JOB_TYPE_CP_UL] = 0;
      ...
      case 8:
        ...
        if ((numRUs >= 2) && (xran_get_syscfg_appmode() == O_RU)) {
          ...
          XRAN_SET_WORKER_INFO(p_worker_info[6], 5, "fh_eth_tx", process_dpdk_io_tx, NULL, 1);
          /** 6 UP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[7],
                               6,
                               "fh_up_gen",
                               xran_tx_symb_ring_processing_func,
                               (void *)(((1L << 1) | (1L << 2) | (1L << 0)) & xran_port_mask),
                               1);
      ...
      case 9:
        ...
        if ((numRUs >= 2) && (xran_get_syscfg_appmode() == O_RU)) {
          ...
          XRAN_SET_WORKER_INFO(p_worker_info[7],
                               ...
                               "fh_up_gen",
                               xran_tx_symb_ring_processing_func,
                               (void *)(((1L << 1)) & xran_port_mask),
                               1);
          /** 7 UP GEN **/
          XRAN_SET_WORKER_INFO(p_worker_info[8],
                               7,
                               "fh_up_gen",
                               xran_tx_symb_ring_processing_func,
                               (void *)(((1L << 2)) & xran_port_mask),
                               1);
  ...
  if (eth_ctx->io_cfg.pkt_proc_core) {
    for (i = 0; i < coreNum && i < 64; i++) {
      if (nWorkerCore & (uint64_t)eth_ctx->io_cfg.pkt_proc_core && eth_ctx->pkt_wrk_cfg[eth_ctx->num_workers].arg != NULL) {
#ifndef POLL_EBBU_OFFLOAD
  ...
  if (eth_ctx->io_cfg.pkt_proc_core_64_127) {
    for (i = 64; i < coreNum && i < 128; i++) {
      if (nWorkerCore & (uint64_t)eth_ctx->io_cfg.pkt_proc_core_64_127 && eth_ctx->pkt_wrk_cfg[eth_ctx->num_workers].arg != NULL) {
#ifndef POLL_EBBU_OFFLOAD
        xran_dev_add_usedcore(i);
        if (rte_eal_remote_launch(eth_ctx->pkt_wrk_cfg[eth_ctx->num_workers].f, eth_ctx->pkt_wrk_cfg[eth_ctx->num_workers].arg, i))
          rte_panic("eth_ctx->pkt_wrk_cfg[eth_ctx->num_workers].f() failed to start\n");
...
int32_t xran_open(void *pHandle, struct xran_fh_config *pConf)
{
  ...
  pDevCtx->perMu = (xran_device_per_mu_fields *)xran_malloc("perMU fields",
                                                            (nNumerology + 1) * sizeof(xran_device_per_mu_fields),
                                                            RTE_CACHE_LINE_SIZE);

  for (i = 0; i < pDevCtx->fh_cfg.numMUs; i++) {
    uint8_t mu = pDevCtx->fh_cfg.mu_number[i];
    pDevCtx->perMu[mu].p_dev_ctx =
        pDevCtx; /* Set the backreference to device context for easy reference when we pass the pointers to perMu[i] around */
  ...
  for (i = 0; i < pDevCtx->fh_cfg.numMUs; i++) {
    ...
    if (pConf->dssEnable) {
      ...
      if (pConf->ru_conf.xranTech == XRAN_RAN_5GNR) {
        ...
        if (mu == XRAN_NBIOT_MU) { /*NBIOT PRACH handling is done inside xran_init_prach API*/
          if ((ret = xran_init_prach(pConf, pDevCtx, XRAN_RAN_5GNR, mu)) < 0)
            return ret;
  ...
  if (nNumerology > xran_timingsource_get_numerology()) {
    printf("RU%d has higher numerology(%d) than current(%d). Updating..\n",
           pConf->dpdk_port,
           nNumerology,
           xran_timingsource_get_numerology());
  ...
  for (i = 0; i < pConf->nCC; i++) {
    ...
    xran_fs_set_slot_type(pConf->dpdk_port,
                          i,
                          pConf->frame_conf.nFrameDuplexType,
                          pConf->frame_conf.nTddPeriod,
...
int32_t xran_start(void *pHandle)
{
  ...
  if (pDevCtx->fh_cfg.numMUs == 1 && pDevCtx->fh_cfg.mu_number[0] == 3 && pDevCtx->vMuInfo.ssbInfo.ssbMu == 4) {
    // Init SSB section DB
    if (xran_cp_init_vMu_sectiondb(pDevCtx,
                                   pDevCtx->vMuInfo.ssbInfo.ssbMu,
                                   pDevCtx->vMuInfo.ssbInfo.ruPortId_offset + xran_get_num_eAxc(pHandle))
        != XRAN_STATUS_SUCCESS) {
  ...
  if (rte_atomic32_test_and_set(&(xran_get_systemcfg()->nStart))) {
    ...
    if (ptm) {
      ...
      printf("%s: XRAN start time: %s.%09ld UTC [%ld]\n",
             (xran_get_syscfg_appmode() == O_DU ? "O-DU" : "O-RU"),
             buff,
             ts.tv_nsec,
             interval_us);
    }
...
int32_t xran_register_cb_mbuf2ring(xran_ethdi_mbuf_send_fn mbuf_send_cp, xran_ethdi_mbuf_send_fn mbuf_send_up)
{
  struct xran_device_ctx *pDevCtx;
...
int32_t xran_timingsource_get_slotidx(uint32_t *nFrameIdx,
                                      uint32_t *nSubframeIdx,
                                      uint32_t *nSlotIdx,
                                      uint64_t *nSecond,
                                      uint8_t mu)
...
int32_t xran_get_slot_idx(__attribute__((unused)) uint32_t PortId,
                          uint32_t *nFrameIdx,
                          uint32_t *nSubframeIdx,
                          uint32_t *nSlotIdx,
                          uint64_t *nSecond,
...
int32_t xran_init_PrbMap_by_symbol_from_cfg(struct xran_prb_map *p_PrbMapIn,
                                            struct xran_prb_map *p_PrbMapOut,
                                            uint32_t mtu,
                                            uint32_t xran_max_prb)
{
...
void xran_l1budget_calc(uint8_t numerology,
                        uint16_t t1a_max_up,
                        uint16_t ta4_max,
                        uint16_t *ul_budget,
                        uint16_t *dl_budget,
...
int32_t xran_fetch_and_print_lbm_stats(bool print_xran_lbm_stats, uint8_t *link_status, uint8_t vfId)
{
  ...
  if (print_xran_lbm_stats) {
    if (xran_get_syscfg_appmode() == ID_O_DU) {
      if (print_xran_lbm_stats) { /* Fetch link status and print lbm stats */
        ...
        printf("\n%10hhu %10hhu %15lu %15lu %15u %15u",
               vfId,
               lbm_port_info->linkStatus,
               lbm_port_info->stats.numTransmittedLBM,
               lbm_port_info->stats.rxValidInOrderLBRs,
               lbm_port_info->stats.rxValidOutOfOrderLBRs,
               lbm_port_info->stats.numRxLBRsIgnored);
        printf("\n");
      }
    } else {
      if (vfId == 0) {
    ...
    if (p_dev_ctx_run) {
      if (p_dev_ctx_run->xran_port_id == xran_port_id) {
        for (i = 0; i < p_dev_ctx_run->fh_cfg.numMUs; i++) {
          ...
          if (XranGetSymNum(pCtx->ebbu_offload_ota_sym_cnt_mu[mu], XRAN_NUM_OF_SYMBOL_PER_SLOT)
              == pCtx->ebbu_offload_ota_sym_idx_mu[mu]) {
