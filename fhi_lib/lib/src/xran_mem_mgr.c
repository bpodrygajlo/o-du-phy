...
int32_t xran_bm_init(void *pHandle, uint32_t *pPoolIndex, uint32_t nNumberOfBuffers, uint32_t nBufferSize)
{
  ...
  snprintf(pool_name,
           ...
           "ru_%d_cc_%d_idx_%d_%d",
           pXranCc->nXranPort,
           pXranCc->nIndex,
           pXranCc->nBufferPoolIndex,
           nDpdkProcessID);

  nAllocBufferSize = nBufferSize + sizeof(struct rte_ether_hdr) + sizeof(struct xran_ecpri_hdr)
                     + sizeof(struct radio_app_common_hdr) + sizeof(struct data_section_hdr) + 256;
  ...
  printf("%s: [ handle %p %d %d ] [nPoolIndex %d] nNumberOfBuffers %d nBufferSize %d socket_id %d\n",
         ...
         rte_socket_id());
#endif
  pXranCc->p_bufferPool[pXranCc->nBufferPoolIndex] = xran_pktmbuf_pool_create(pool_name,
                                                                              nNumberOfBuffers,
                                                                              /*MBUF_CACHE*/ 0,
                                                                              0,
                                                                              nAllocBufferSize,
  ...
  if (pXranCc->p_bufferPool[pXranCc->nBufferPoolIndex] == NULL) {
    rte_panic(
        "rte_pktmbuf_pool_create failed [poolName=%s, handle %p %d %d ] [nPoolIndex %d] nNumberOfBuffers %d nBufferSize %d errno "
        "%s\n",
        pool_name,
        pXranCc,
        pXranCc->nXranPort,
  ...
  *ppCtrl = NULL;
  ...
  if (mb) {
    ...
    if (start && ethhdr) {
      ...
      /* skip headers */
      iq_offset = iq_offset + sizeof(struct rte_ether_hdr) + sizeof(struct xran_ecpri_hdr) + sizeof(struct radio_app_common_hdr)
...
int32_t xran_bm_free_ring(void *pHandle, void *pRing)
{
  if (pHandle && pRing) {
    ...
    /*  //for some reason the xran_bm_allocate_ring() did not use rte_ring_create to create ring, thus here we should not use
       rte_ring_free to free it gpxRANRemoveFn(ring); rte_ring_free(ring);*/
  } else {
    return (-1);
  }
