...
/* MAC to PHY API prach numerology (nPrachSubcSpacing) 0-4  0:15kHz  1:30kHz  2:60kHz  3:120kHz  4:1.25kHz, currently 5kHz not
 * used*/
...
int32_t xran_cp_init_vMu_sectiondb(void *pHandle, uint8_t vMu, int32_t nAnt)
{
  struct xran_device_ctx *p_dev = NULL;
  uint8_t xran_port_id = 0;
  PSECTION_DB_TYPE p_sec_db = NULL;
...
struct xran_section_info *xran_cp_get_section_info_ptr(void *pHandle,
                                                       uint8_t dir,
                                                       uint8_t cc_id,
                                                       uint8_t ruport_id,
                                                       uint8_t ctx_id,
 ...
 */
int32_t xran_cp_reset_section_info(void *pHandle, uint8_t dir, uint8_t cc_id, uint8_t ruport_id, uint8_t ctx_id, uint8_t mu)
{
  struct xran_sectioninfo_db *ptr;
 ...
 **/
//-------------------------------------------------------------------------------------------
int32_t xran_get_prach_freqoffset(struct xran_fh_config *pConf, uint8_t mu, uint8_t prachMu, uint16_t prachStartPrb)
{
   ...
   * freqOffset indicate the location of lowest RE's center in the lowest RB defined by frameStructure, with respect to
   * center-of-channel-bandwidth prachStartPrb (nPrachFreqStart) from mac2phy API, range 0 - 272 ORAN prach freqoffset (in 0.5 x
   * PRACH SCS) = 0 - (bandwidth/2) + startprb_offset MAC does not config additional PRACH FreqOffset */
...
static int32_t xran_append_sectionext_1(struct rte_mbuf *mbuf, struct xran_sectionext1_info *params, int32_t last_flag)
{
  int32_t total_len = 0;
...
static int32_t xran_prepare_sectionext_2(struct rte_mbuf *mbuf, struct xran_sectionext2_info *params, int32_t last_flag)
{
...
static int32_t xran_prepare_sectionext_3(struct rte_mbuf *mbuf, struct xran_sectionext3_info *params, int32_t last_flag)
{
  int32_t total_len;
  int32_t adj;
  ...
  if (params->layerId == XRAN_LAYERID_0 || params->layerId == XRAN_LAYERID_TXD) { /* first data layer */
    ...
    data_first_byte =
        (params->txScheme << xran_cp_radioapp_sec_ext3_TxScheme) | (params->crsReMask << xran_cp_radioapp_sec_ext3_CrcReMask)
        | (params->crsShift << xran_cp_radioapp_sec_ext3_CrcShift) | (params->crsSymNum << xran_cp_radioapp_sec_ext3_CrcSymNum);
    data_second_byte =
        (last_flag << xran_cp_radioapp_sec_ext3_EF) | (XRAN_CP_SECTIONEXTCMD_3 << xran_cp_radioapp_sec_ext3_ExtType)
        | (extLen << xran_cp_radioapp_sec_ext3_ExtLen) | (params->codebookIdx << xran_cp_radioapp_sec_ext3_CodebookIdx)
        | (params->layerId << xran_cp_radioapp_sec_ext3_LayerId) | (params->numLayers << xran_cp_radioapp_sec_ext3_NumLayers);
    ...
    *tmp = rte_cpu_to_be_64(*tmp);
    ...
    ext3_nf->data_field =
        ...
        | (params->codebookIdx << xran_cp_radioapp_sec_ext3_CodebookIdx) | (params->layerId << xran_cp_radioapp_sec_ext3_LayerId)
...
static int32_t xran_prepare_sectionext_4(struct rte_mbuf *mbuf, struct xran_sectionext4_info *params, int32_t last_flag)
{
...
static int32_t xran_prepare_sectionext_9(struct rte_mbuf *mbuf, struct xran_sectionext9_info *params, int32_t last_flag)
{
...
static int32_t xran_prepare_sectionext_5(struct rte_mbuf *mbuf, struct xran_sectionext5_info *params, int32_t last_flag)
{
...
static int32_t xran_prepare_sectionext_6(struct rte_mbuf *mbuf, struct xran_sectionext6_info *params, int32_t last_flag)
{
...
static int32_t xran_prepare_sectionext_10(struct rte_mbuf *mbuf, struct xran_sectionext10_info *params, int32_t last_flag)
{
  ...
  if (params->beamGrpType != XRAN_BEAMGT_COMMON && params->beamGrpType != XRAN_BEAMGT_MATRIXIND
 ...
 */
int32_t xran_cp_estimate_max_set_bfws(uint8_t numBFWs, uint8_t iqWidth, uint8_t compMeth, uint16_t mtu)
{
  int32_t avail_len;
  int32_t bfw_bitsize;
  int32_t bundle_size;
  ...
  avail_len = mtu
              - (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct xran_cp_radioapp_section1_header)
                 + sizeof(struct xran_cp_radioapp_section1) + sizeof(union xran_cp_radioapp_section_ext6)
...
inline static uint32_t xran_cp_get_hdroffset_section1(uint32_t exthdr_size)
{
  uint32_t hdr_len;

  hdr_len = (RTE_PKTMBUF_HEADROOM + sizeof(struct xran_ecpri_hdr) + sizeof(struct xran_cp_radioapp_section1_header)
             + sizeof(struct xran_cp_radioapp_section1) + exthdr_size);
  return (hdr_len);
}
...
int32_t xran_cp_prepare_ext11_bfws(uint8_t numSetBFW,
                                   uint8_t numBFW,
                                   uint8_t iqWidth,
                                   uint8_t compMeth,
                                   uint8_t *dst,
...
#endif
{
  ...
  if (total_len >= dst_maxlen) {
    print_err("Exceed maximum length to fit the set of BFWs - (%d/%d)", total_len, dst_maxlen);
    return (XRAN_STATUS_RESOURCE);
  }
  ...
  parm_size = (total_len + sizeof(union xran_cp_radioapp_section_ext11)) % XRAN_SECTIONEXT_ALIGN;
  if (parm_size) {
    /* Add padding */
    parm_size = XRAN_SECTIONEXT_ALIGN - parm_size;
...
static int32_t xran_prepare_sectionext_11(struct rte_mbuf *mbuf, struct xran_sectionext11_info *params, int32_t last_flag)
{
  ...
  ext11->data_field.data_field2 = ((XRAN_CONVERT_BFWIQWIDTH(params->bfwIqWidth)) << xran_cp_radioapp_sec_ext11_bitfield_BFWIQWidth)
                                  | (params->bfwCompMeth << xran_cp_radioapp_sec_ext11_bitfield_BFWCompMeth) | params->numBundPrb;
 ...
 */
static int32_t xran_prepare_section0(struct xran_cp_radioapp_section0 *section, struct xran_section_gen_info *params, uint8_t mu)
{
#if (XRAN_STRICT_PARM_CHECK)
 ...
 */
static int32_t xran_prepare_section0_hdr(struct xran_cp_radioapp_section0_header *s0hdr, struct xran_cp_gen_params *params)
{
 ...
 *  XRAN_STATUS_INVALID_PARM if the number of symbol is invalid
 */
static int32_t xran_prepare_section1(struct xran_cp_radioapp_section1 *section, struct xran_section_gen_info *params, uint8_t mu)
{
#if (XRAN_STRICT_PARM_CHECK)
  if (unlikely(params->info->numSymbol > XRAN_SYMBOLNUMBER_MAX)) {
    print_err("Invalid number of Symbols - %d", params->info->numSymbol);
  ...
  section->hdr.u.first_4byte =
      (params->info->reMask << xran_cp_radioapp_sec_hdr_sc_ReMask) | (params->info->numSymbol << xran_cp_radioapp_sec_hdr_sc_NumSym)
      | (params->info->ef << xran_cp_radioapp_sec_hdr_sc_Ef) | (params->info->beamId << xran_cp_radioapp_sec_hdr_sc_BeamID);
 ...
 *  XRAN_STATUS_SUCCESS always
 */
static int32_t xran_prepare_section1_hdr(struct xran_cp_radioapp_section1_header *s1hdr, struct xran_cp_gen_params *params)
{
 ...
 *  XRAN_STATUS_INVALID_PARM if the number of symbol is invalid
 */
static int32_t xran_prepare_section3(struct xran_cp_radioapp_section3 *section, struct xran_section_gen_info *params, uint8_t mu)
{
#if (XRAN_STRICT_PARM_CHECK)
  if (unlikely(params->info->numSymbol > XRAN_SYMBOLNUMBER_MAX)) {
    print_err("Invalid number of Symbols - %d", params->info->numSymbol);
 ...
 *  XRAN_STATUS_SUCCESS always
 */
static int32_t xran_prepare_section3_hdr(struct xran_cp_radioapp_section3_header *s3hdr, struct xran_cp_gen_params *params)
{
 ...
 *  XRAN_STATUS_RESOURCE if failed to allocate the space to packet buffer
 */
int32_t xran_append_control_section(struct rte_mbuf *mbuf, struct xran_cp_gen_params *params, uint16_t start_sect_id, uint8_t mu)
{
  int32_t i, ret;
  uint32_t totalen;
  void *section;
  ...
  for (i = start_sect_id; i < (start_sect_id + params->numSections); i++) {
    ...
    ret = xran_prepare_section_func((void *)section, (void *)&params->sections[i], mu);
    if (ret < 0) {
      print_err("%s %d\n", __FUNCTION__, ret);
      return (ret);
    }
    ...
    if (params->sections[i].info->ef) {
      print_dbg("sections[%d].info.ef %d exDataSize %d  type %d\n",
                i,
                params->sections[i].info->ef,
                params->sections[i].exDataSize,
                params->sections[i].exData[0].type);
 ...
 */
static inline int32_t xran_prepare_radioapp_common_header(struct xran_cp_radioapp_common_header *apphdr,
 ...
 *  XRAN_STATUS_RESOURCE if failed to allocate the space to packet buffer
 */
int32_t xran_append_radioapp_header(struct rte_mbuf *mbuf, struct xran_cp_gen_params *params)
{
  int32_t ret;
  uint32_t totalen;
...
///////////////////////////////////////
// for RU emulation
int32_t xran_parse_section_ext1(void *ext, struct xran_sectionext1_info *extinfo)
{
  int32_t len;
  int32_t total_len;
...
int32_t xran_parse_section_ext2(void *ext, struct xran_sectionext2_info *extinfo)
{
  int32_t len;
  int32_t total_len;
  struct xran_cp_radioapp_section_ext2 *ext2;
  ...
  *(uint32_t *)ext2 = rte_be_to_cpu_32(*(uint32_t *)ext2);
  ...
  val_size = (extinfo->bfAzPtWidth ? extinfo->bfAzPtWidth + 1 : 0) + (extinfo->bfZePtWidth ? extinfo->bfZePtWidth + 1 : 0)
             + (extinfo->bfAz3ddWidth ? extinfo->bfAz3ddWidth + 1 : 0) + (extinfo->bfZe3ddWidth ? extinfo->bfZe3ddWidth + 1 : 0);
  if (val_size) {
...
int32_t xran_parse_section_ext3(void *ext, struct xran_sectionext3_info *extinfo)
{
  int32_t len;
  int32_t total_len;
...
int32_t xran_parse_section_ext4(void *ext, struct xran_sectionext4_info *extinfo)
{
  int32_t len;
  struct xran_cp_radioapp_section_ext4 *ext4;
  int32_t total_len;
...
int32_t xran_parse_section_ext5(void *ext, struct xran_sectionext5_info *extinfo)
{
  struct xran_cp_radioapp_section_ext_hdr *ext_hdr;
  struct xran_cp_radioapp_section_ext5 ext5;
  int32_t parm_size;
...
int32_t xran_parse_section_ext6(void *ext, struct xran_sectionext6_info *extinfo)
{
  int32_t len;
  union xran_cp_radioapp_section_ext6 *ext6;
  int32_t total_len;
...
int32_t xran_parse_section_ext9(void *ext, struct xran_sectionext9_info *extinfo, struct xran_cp_recv_params *result)
{
  int32_t len = 0;
  int32_t total_len;
  int8_t dssSlot = 0;
...
int32_t xran_parse_section_ext11(void *ext, struct xran_sectionext11_recv_info *extinfo)
{
  int32_t len;
  int32_t total_len;
  union xran_cp_radioapp_section_ext11 *ext11;
...
int32_t xran_parse_section_extension(struct rte_mbuf *mbuf, void *ext, struct xran_cp_recv_params *result, int32_t section_idx)
{
  ...
  while (flag_last) {
    ...
    // i++;
    if (++numext < XRAN_MAX_NUM_EXTENSIONS)
      continue;
                          ...
                          uint32_t *mb_free)
{
  ...
  *((uint32_t *)apphdr) = rte_be_to_cpu_32(*((uint32_t *)apphdr));
  ...
  if (likely(XRAN_CP_SECTIONTYPE_3 != result->sectionType)) {
    ...
    tti = apphdr->field.frameId * SLOTS_PER_SYSTEMFRAME(interval) + apphdr->field.subframeId * SLOTNUM_PER_SUBFRAME(interval)
          + apphdr->field.slotId;
    result->tti = tti;
    ...
    case XRAN_CP_SECTIONTYPE_0: // Unused RB or Symbols in DL or UL, not supportted
    {
      ...
      for (i = 0; i < result->numSections; i++) {
        *((uint64_t *)section) = rte_be_to_cpu_64(*((uint64_t *)section));
        ...
        if (section == NULL) {
          print_dbg("Invalid packet: section type0 - number of section [%d:%d]!", result->numSections, i);
    ...
    case XRAN_CP_SECTIONTYPE_1: // Most DL/UL Radio Channels
    {
      ...
      for (i = 0; i < result->numSections; i++) {
        *((uint64_t *)section) = rte_be_to_cpu_64(*((uint64_t *)section));
        ...
        section = (void *)rte_pktmbuf_adj(mbuf, sizeof(struct xran_cp_radioapp_section1));
        if (unlikely(section == NULL)) {
          print_dbg("Invalid packet: section type1 - number of section [%d:%d]!", result->numSections, i);
          result->numSections = i;
          ret = XRAN_STATUS_INVALID_PACKET;
          break;
        }
        ...
        if (eaxc.ruPortId >= eaxcOffset && eaxc.ruPortId < (eaxcOffset + xran_get_num_eAxc(p_xran_dev_ctx))) {
          ...
          if (result->dir == XRAN_DIR_DL) {
            pBuffer = p_xran_dev_ctx->perMu[mu]
                          .sFHCpRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
                          .sBufferList.pBuffers;
          } else if (result->dir == XRAN_DIR_UL) {
            pBuffer = p_xran_dev_ctx->perMu[mu]
                          .sFHCpTxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
                          .sBufferList.pBuffers;
          }
          ...
          if (p_xran_dev_ctx->perMu[mu]
                  .sFrontHaulTxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
                  .sBufferList.pBuffers)
            pRbMap_desc = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                              .sFrontHaulTxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
          ...
          if (i == 0) {
            if ((pRbMap_desc != NULL)
                && (pRbMap_desc->nPrbElm
                    <= p_xran_dev_ctx->perMu[mu].sectiondb_elm[ctx_id][result->dir][eaxc.ccId][eaxc.ruPortId])) {
          ...
          if (p_xran_dev_ctx->perMu[mu]
                  .sFHSrsRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
                  .sBufferList.pBuffers) {
            pRbMap_desc = (struct xran_prb_map *)p_xran_dev_ctx->perMu[mu]
                              .sFHSrsRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
                              .sBufferList.pBuffers->pData;
      ...
      if (pCnt && (result->sections[0].info.ef) && (result->sections[0].exts[0].type == 1)
          && (result->numSections != result->numSetBFW) && (result->ext1count != result->numSetBFW)) {
    ...
    case XRAN_CP_SECTIONTYPE_3: // PRACH and Mixed-numerology Channels
    {
      ...
      for (i = 0; i < result->numSections; i++) {
        *((uint64_t *)section) = rte_be_to_cpu_64(*((uint64_t *)section));
        ...
        if (unlikely(section == NULL)) {
          print_dbg("Invalid packet: section type3 - number of section [%d:%d]!", result->numSections, i);
          result->numSections = i;
          ret = XRAN_STATUS_INVALID_PACKET;
          break;
        }
        ...
        if (isPusch) {
          ...
          if (eaxc.ruPortId >= eaxcOffset && eaxc.ruPortId < (eaxcOffset + xran_get_num_eAxc(p_xran_dev_ctx))) { // pusch
            struct xran_flat_buffer *pBuffer = NULL;
            if (result->dir == XRAN_DIR_DL) {
              pBuffer = p_xran_dev_ctx->perMu[mu]
                            .sFHCpRxPrbMapBbuIoBufCtrl[tti % XRAN_N_FE_BUF_LEN][eaxc.ccId][ant_id]
