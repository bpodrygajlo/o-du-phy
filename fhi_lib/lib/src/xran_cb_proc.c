...
void xran_timer_arm_cp_dl(struct rte_timer *tim, void *arg, void *p_dev_ctx, uint8_t mu)
{
  ...
  if (xran_if_current_state == XRAN_RUNNING) {
    ...
    pCtx->pFn(p_xran_dev_ctx->xran_port_id,
              TMNG_DL_CP_POLL,
              0,
              &(p_xran_dev_ctx->perMu[mu])); /* Generate Task TMNG_DL_CP_POLL with no delay*/
#else
...
void xran_timer_arm_cp_ul(struct rte_timer *tim, void *arg, void *p_dev_ctx, uint8_t mu)
{
  ...
  if (xran_if_current_state == XRAN_RUNNING) {
    ...
    pCtx->pFn(p_xran_dev_ctx->xran_port_id,
              TMNG_UL_CP_POLL,
              0,
              &(p_xran_dev_ctx->perMu[mu])); /* Generate Task TMNG_UL_CP_POLL with no delay*/
#else
...
void xran_timer_arm_for_deadline(struct rte_timer *tim, void *arg, void *p_dev_ctx, uint8_t mu)
{
  ...
  rx_tti = nFrameIdx * SUBFRAMES_PER_SYSTEMFRAME * SLOTNUM_PER_SUBFRAME(xran_fs_get_tti_interval(mu))
           + nSubframeIdx * SLOTNUM_PER_SUBFRAME(xran_fs_get_tti_interval(mu)) + nSlotIdx;
...
int32_t xran_timing_create_cbs(void *args)
{
  ...
  for (int32_t i = 0; i < p_dev_ctx->fh_cfg.numMUs; i++) {
    ...
    if (xran_get_syscfg_appmode() == O_DU) {
      ...
      printf(
          "RU%d mu%u, delay_cp_dl_max=%u, sym_cp_dl_max=%u, max_dl_offset_sym=%u\n"
      ...
      lower_bound_window =
          (p_dev_ctx->fh_cfg.perMu[mu].Ta3_min > p_dev_ctx->fh_cfg.perMu[mu].Ta4_min ? p_dev_ctx->fh_cfg.perMu[mu].Ta3_min
                                                                                     : p_dev_ctx->fh_cfg.perMu[mu].Ta4_min);
      ...
      if (xran_get_syscfg_bbuoffload()) {
        ...
        p_dev_ctx->perMu[mu].adv_tx_factor = (p_dev_ctx->fh_cfg.perMu[mu].adv_tx_time)
                                                 ? (interval_us_local / (p_dev_ctx->fh_cfg.perMu[mu].adv_tx_time * N_SYM_PER_SLOT))
                                                 : 0;
        ...
        while (count < p_dev_ctx->numSymsForDlCP) {
          ...
          if (cb_elm) {
            LIST_INSERT_HEAD(&p_dev_ctx->perMu[mu].sym_cb_list_head[sym_cp_dl_max], cb_elm, pointers);
          } else {
        ...
        if (cb_elm) {
          LIST_INSERT_HEAD(&p_dev_ctx->perMu[mu].sym_cb_list_head[sym_cp_ul], cb_elm, pointers);
        } else {
          print_err("cb_elm is NULL\n");
      ...
      if (cb_elm) {
        LIST_INSERT_HEAD(&p_dev_ctx->perMu[mu].sym_cb_list_head[time_diff_nSymb % XRAN_NUM_OF_SYMBOL_PER_SLOT], cb_elm, pointers);
      } else {
        print_err("cb_elm is NULL\n");
        goto err0;
      }
      ...
      if (time_diff_nSymb + 1 * (N_SYM_PER_SLOT / 4) > N_SYM_PER_SLOT) {
        p_dev_ctx->perMu[mu].deadline_slot_advance[XRAN_SLOT_1_4_CB] =
            (time_diff_nSymb + 1 * (N_SYM_PER_SLOT / 4)) / N_SYM_PER_SLOT;
      }
      ...
      if (cb_elm) {
        LIST_INSERT_HEAD(
            &p_dev_ctx->perMu[mu].sym_cb_list_head[(time_diff_nSymb + 1 * (N_SYM_PER_SLOT / 4)) % XRAN_NUM_OF_SYMBOL_PER_SLOT],
            cb_elm,
            pointers);
      } else {
        print_err("cb_elm is NULL\n");
      ...
      if (cb_elm) {
        LIST_INSERT_HEAD(
            &p_dev_ctx->perMu[mu].sym_cb_list_head[(time_diff_nSymb + N_SYM_PER_SLOT / 2) % XRAN_NUM_OF_SYMBOL_PER_SLOT],
            cb_elm,
            pointers);
      } else {
        print_err("cb_elm is NULL\n");
      ...
      if (time_diff_nSymb + 4 * (N_SYM_PER_SLOT / 4)) {
        p_dev_ctx->perMu[mu].deadline_slot_advance[XRAN_SLOT_3_4_CB] =
            (time_diff_nSymb + 4 * (N_SYM_PER_SLOT / 4)) / N_SYM_PER_SLOT;
      }
      ...
      if (cb_elm) {
        LIST_INSERT_HEAD(
            &p_dev_ctx->perMu[mu].sym_cb_list_head[(time_diff_nSymb + 4 * (N_SYM_PER_SLOT / 4)) % XRAN_NUM_OF_SYMBOL_PER_SLOT],
            cb_elm,
            pointers);
      } else {
        print_err("cb_elm is NULL\n");
      ...
      printf("RU%d T2a_min_up=%u, T2a_max_up=%u, ruRxSymUpMin=%u, ruRxSymUpMax=%u\n",
             ru_id,
             p_dev_ctx->fh_cfg.perMu[mu].T2a_min_up,
             p_dev_ctx->fh_cfg.perMu[mu].T2a_max_up,
             p_dev_ctx->perMu[mu].ruRxSymUpMin,
             p_dev_ctx->perMu[mu].ruRxSymUpMax);

      /* Full slot UL OTA + time_diff_us */
      cb_elm = xran_create_cb(xran_timer_arm_for_deadline, rx_ul_deadline_full_cb, (void *)p_dev_ctx);
      if (cb_elm) {
        LIST_INSERT_HEAD(&p_dev_ctx->perMu[mu].sym_cb_list_head[sym_up % XRAN_NUM_OF_SYMBOL_PER_SLOT], cb_elm, pointers);
      } else {
        print_err("cb_elm is NULL\n");
        goto err0;
      }
...
static int32_t xran_reg_sym_cb_ota(struct xran_device_ctx *p_dev_ctx,
...
static int32_t xran_reg_sym_cb_rx_win_end(struct xran_device_ctx *p_dev_ctx,
                                          xran_callback_sym_fn symCb,
                                          void *symCbParam,
                                          struct xran_sense_of_time *symCbTime,
                                          uint8_t symb,
...
static int32_t xran_reg_sym_cb_rx_win_begin(struct xran_device_ctx *p_dev_ctx,
                                            xran_callback_sym_fn symCb,
                                            void *symCbParam,
                                            struct xran_sense_of_time *symCbTime,
                                            uint8_t symb,
...
static int32_t xran_reg_sym_cb_tx_win_end(struct xran_device_ctx *p_dev_ctx,
                                          xran_callback_sym_fn symCb,
                                          void *symCbParam,
                                          struct xran_sense_of_time *symCbTime,
                                          uint8_t symb,
...
static int32_t xran_reg_sym_cb_tx_win_begin(struct xran_device_ctx *p_dev_ctx,
                                            xran_callback_sym_fn symCb,
                                            void *symCbParam,
                                            struct xran_sense_of_time *symCbTime,
                                            uint8_t symb,
                        ...
                        uint8_t mu)
{
  ...
  if (cb_elm) {
    LIST_INSERT_HEAD(&p_dev_ctx->perMu[mu].sym_cb_list_head[p_sym_cb_ctx->symb_num_ota], cb_elm, pointers);
  } else {
    print_err("cb_elm is NULL\n");
