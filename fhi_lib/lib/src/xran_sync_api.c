...
static int find_substr(const char *str, const unsigned int str_len, const char *substr, const unsigned int substr_len)
{
  assert(str);
  assert(substr);
  unsigned int ind = 0;
...
static int check_ptp_status()
{
  ...
  while (fgets(pmc_out_line, BUF_LEN, pmc_pipe)) {
    if (0 == find_substr(pmc_out_line, strlen(pmc_out_line), keywords[0], strlen(keywords[0]))) {
      if (0 == find_substr(pmc_out_line, strlen(pmc_out_line), keywords[1], strlen(keywords[1]))) {
        res = 0;
        break;
      }
    }
