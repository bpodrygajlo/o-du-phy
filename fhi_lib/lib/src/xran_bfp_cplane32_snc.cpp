 ...
 *
 * @file xran_bfp_cplane32_snc.cpp
...
inline int maxAbsOneBlock(const __m512i* inData)
{
...
inline __m512i computeExponent_16RB(const BlockFloatCompander::ExpandedData& dataIn, const __m512i totShiftBits)
{
...
inline __m512i computeExponent_4RB(const BlockFloatCompander::ExpandedData& dataIn, const __m512i totShiftBits)
{
  __m512i maxAbs = __m512i();
  const __m512i* dataInAddr = reinterpret_cast<const __m512i*>(dataIn.dataExpanded);
#pragma unroll(4)
...
inline uint8_t computeExponent_1RB(const BlockFloatCompander::ExpandedData& dataIn, const __m512i totShiftBits)
{
  __m512i maxAbs = __m512i();
  const __m512i* dataInAddr = reinterpret_cast<const __m512i*>(dataIn.dataExpanded);
...
inline void applyCompressionN_1RB(const __m512i* dataIn,
                                  uint8_t* outBlockAddr,
                                  const int iqWidth,
                                  const uint8_t thisExp,
                                  const int totNumBytesPerReg,
                                  const uint64_t rbWriteMask)
{
...
template <BlockFloatCompander::PackFunction networkBytePack>
inline void compressN_16RB(const BlockFloatCompander::ExpandedData& dataIn,
                           BlockFloatCompander::CompressedData* dataOut,
                           const __m512i totShiftBits,
                           const int totNumBytesPerBlock,
                           const int totNumBytesPerReg,
                           const uint64_t rbWriteMask)
{
...
#pragma unroll(16)
  for (int n = 0; n < 16; ++n) {
    applyCompressionN_1RB<networkBytePack>(dataInAddr + n * k_numRegsPerBlock,
                                           dataOut->dataCompressed + n * totNumBytesPerBlock,
                                           dataIn.iqWidth,
                                           ((uint8_t*)&exponents)[n * 4],
                                           totNumBytesPerReg,
                                           rbWriteMask);
  }
}
...
template <BlockFloatCompander::PackFunction networkBytePack>
inline void compressN_4RB(const BlockFloatCompander::ExpandedData& dataIn,
                          BlockFloatCompander::CompressedData* dataOut,
                          const __m512i totShiftBits,
                          const int totNumBytesPerBlock,
                          const int totNumBytesPerReg,
...
template <BlockFloatCompander::PackFunction networkBytePack>
inline void compressN_1RB(const BlockFloatCompander::ExpandedData& dataIn,
                          BlockFloatCompander::CompressedData* dataOut,
                          const __m512i totShiftBits,
                          const int totNumBytesPerBlock,
                          const int totNumBytesPerReg,
                          const uint64_t rbWriteMask)
{
  ...
  const __m512i* dataInAddr = reinterpret_cast<const __m512i*>(dataIn.dataExpanded);
  applyCompressionN_1RB<networkBytePack>(dataInAddr,
                                         dataOut->dataCompressed,
                                         dataIn.iqWidth,
                                         thisExponent,
                                         totNumBytesPerReg,
                                         rbWriteMask);
...
template <BlockFloatCompander::PackFunction networkBytePack>
inline void compressByAllocN(const BlockFloatCompander::ExpandedData& dataIn,
                             BlockFloatCompander::CompressedData* dataOut,
                             const __m512i totShiftBits,
                             const int totNumBytesPerBlock,
                             const int totNumBytesPerReg,
...
inline void applyCompression8_1RB(const __m512i* dataIn, uint8_t* outBlockAddr, const uint8_t thisExp)
{
  /// Store exponent first
  *outBlockAddr = thisExp;
...
inline void compress8_16RB(const BlockFloatCompander::ExpandedData& dataIn,
                           BlockFloatCompander::CompressedData* dataOut,
                           const __m512i totShiftBits)
{
  const auto exponents = computeExponent_16RB(dataIn, totShiftBits);
...
#pragma unroll(16)
  for (int n = 0; n < 16; ++n) {
    applyCompression8_1RB(dataInAddr + n * k_numRegsPerBlock,
                          dataOut->dataCompressed + n * (k_numDataElements + 1),
                          ((uint8_t*)&exponents)[n * 4]);
  }
}

/// Derive and apply 8b compression to 4 compression blocks
inline void compress8_4RB(const BlockFloatCompander::ExpandedData& dataIn,
                          BlockFloatCompander::CompressedData* dataOut,
                          const __m512i totShiftBits)
{
  const auto exponents = computeExponent_4RB(dataIn, totShiftBits);
...
inline void compress8_1RB(const BlockFloatCompander::ExpandedData& dataIn,
                          BlockFloatCompander::CompressedData* dataOut,
                          const __m512i totShiftBits)
{
  const auto thisExponent = computeExponent_1RB(dataIn, totShiftBits);
...
inline void compressByAlloc8(const BlockFloatCompander::ExpandedData& dataIn,
                             BlockFloatCompander::CompressedData* dataOut,
                             const __m512i totShiftBits)
{
  switch (dataIn.numBlocks) {
...
inline void applyExpansionN_1RB(const uint8_t* expAddr, __m512i* dataOutAddr, const int maxExpShift, const int totNumBytesPerReg)
{
...
template <BlockFloatCompander::UnpackFunction networkByteUnpack>
void expandByAllocN(const BlockFloatCompander::CompressedData& dataIn,
                    BlockFloatCompander::ExpandedData* dataOut,
                    const int totNumBytesPerBlock,
                    const int totNumBytesPerReg,
                    const int maxExpShift)
{
...
#pragma unroll(16)
      for (int n = 0; n < 16; ++n) {
        applyExpansionN_1RB<networkByteUnpack>(dataIn.dataCompressed + n * totNumBytesPerBlock,
                                               dataOutAddr + n * k_numRegsPerBlock,
                                               maxExpShift,
                                               totNumBytesPerReg);
      }
      break;
...
inline void applyExpansion8_1RB(const uint8_t* expAddr, __m512i* dataOutAddr)
{
...
void expandByAlloc8(const BlockFloatCompander::CompressedData& dataIn, BlockFloatCompander::ExpandedData* dataOut)
{
  __m512i* dataOutAddr = reinterpret_cast<__m512i*>(dataOut->dataExpanded);
  switch (dataIn.numBlocks) {
    case 16:
...
void BlockFloatCompander::BFPCompressCtrlPlane32AvxSnc(const ExpandedData& dataIn, CompressedData* dataOut)
{
  ...
  const auto totNumBytesPerBlock = ((BFP_CPlane_32_SNC::k_numDataElements * dataIn.iqWidth) >> 3) + 1;
  ...
  switch (dataIn.iqWidth) {
    case 8:
      BFP_CPlane_32_SNC::compressByAlloc8(dataIn, dataOut, totShiftBits8);
      break;

    case 9:
      BFP_CPlane_32_SNC::compressByAllocN<BlockFloatCompander::networkBytePack9bSnc>(dataIn,
                                                                                     dataOut,
                                                                                     totShiftBits9,
                                                                                     totNumBytesPerBlock,
                                                                                     totNumBytesPerReg,
    ...
    case 10:
      BFP_CPlane_32_SNC::compressByAllocN<BlockFloatCompander::networkBytePack10bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totShiftBits10,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
    ...
    case 12:
      BFP_CPlane_32_SNC::compressByAllocN<BlockFloatCompander::networkBytePack12bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totShiftBits12,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
    ...
    case 14:
      BFP_CPlane_32_SNC::compressByAllocN<BlockFloatCompander::networkBytePack14bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totShiftBits14,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
...
/// Starts by determining iqWidth specific parameters and functions.
void BlockFloatCompander::BFPExpandCtrlPlane32AvxSnc(const CompressedData& dataIn, ExpandedData* dataOut)
{
  ...
  switch (dataIn.iqWidth) {
    case 8:
      BFP_CPlane_32_SNC::expandByAlloc8(dataIn, dataOut);
      break;

    case 9:
      BFP_CPlane_32_SNC::expandByAllocN<BlockFloatCompander::networkByteUnpack9bSnc>(dataIn,
                                                                                     dataOut,
                                                                                     totNumBytesPerBlock,
                                                                                     totNumBytesPerReg,
                                                                                     k_maxExpShift9);
    ...
    case 10:
      BFP_CPlane_32_SNC::expandByAllocN<BlockFloatCompander::networkByteUnpack10bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
                                                                                      k_maxExpShift10);
    ...
    case 12:
      BFP_CPlane_32_SNC::expandByAllocN<BlockFloatCompander::networkByteUnpack12bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
                                                                                      k_maxExpShift12);
    ...
    case 14:
      BFP_CPlane_32_SNC::expandByAllocN<BlockFloatCompander::networkByteUnpack14bSnc>(dataIn,
                                                                                      dataOut,
                                                                                      totNumBytesPerBlock,
                                                                                      totNumBytesPerReg,
                                                                                      k_maxExpShift14);
